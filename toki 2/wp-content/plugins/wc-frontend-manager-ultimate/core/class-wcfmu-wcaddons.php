...
class WCFMu_WCAddons
{
        ...
        if (apply_filters('wcfm_is_pref_wc_product_addon', true) && apply_filters('wcfm_is_allow_wc_product_addon', true)) {
            if (WCFMu_Dependencies::wcfm_wc_addons_active_check() || WCFMu_Dependencies::wcfm_wc_appointments_active_check()) {
                ...
                if (( defined('WC_PRODUCT_ADDONS_VERSION') && version_compare(WC_PRODUCT_ADDONS_VERSION, '3.0.0', '>=') ) || WCFMu_Dependencies::wcfm_wc_appointments_active_check()) {
                    ...
                    add_action('end_wcfm_products_manage', [ &$this, 'wcaddons_wcfm_products_manage_3' ], 210);

                    // WC Product Addons Load WCFMu Scripts
                    add_action('wcfm_load_scripts', [ &$this, 'wcaddons_load_scripts_3' ], 90);
                    add_action('after_wcfm_load_scripts', [ &$this, 'wcaddons_load_scripts_3' ], 90);

                    // WC Product Addons Product Manage View
                    add_action('after_wcfm_products_manage_meta_save', [ &$this, 'wcaddons_wcfm_products_manage_meta_save_3' ], 200, 2);
                } else {
                    // WC Product Addons Product Manage View
                    add_action('end_wcfm_products_manage', [ &$this, 'wcaddons_wcfm_products_manage' ], 210);

                    // WC Product Addons Load WCFMu Scripts
                    add_action('wcfm_load_scripts', [ &$this, 'wcaddons_load_scripts' ], 90);
                    add_action('after_wcfm_load_scripts', [ &$this, 'wcaddons_load_scripts' ], 90);

                    // WC Product Addons Product Manage View
                    add_action('after_wcfm_products_manage_meta_save', [ &$this, 'wcaddons_wcfm_products_manage_meta_save' ], 200, 2);
                }//end if
            }//end if
        }//end if
    ...
    function wcaddons_wcfm_products_manage_3($product_id)
    {
        ...
        if ($product_id) {
            ...
            $product        = '';
            $exists         = false;
            $product_addons = apply_filters('wcfm_wc_product_addon_elements', [], $product_id);
            $exclude_global = 0;
        }
    ...
    }//end wcaddons_wcfm_products_manage_3()
        ...
        switch ($end_point) {
            case 'wcfm-products-manage':
                ...
                if (WCFMu_Dependencies::wcfm_wc_appointments_active_check()) {
                    wp_enqueue_style('wc_appointments_admin_styles', WC_APPOINTMENTS_PLUGIN_URL.'/assets/css/admin.css', [ 'woocommerce_product_addons_css' ], WC_APPOINTMENTS_VERSION);
                }

                $suffix = defined('SCRIPT_DEBUG') && SCRIPT_DEBUG ? '' : '.min';

                wp_register_script('woocommerce_product_addons', plugins_url('assets/js/admin'.$suffix.'.js', WC_PRODUCT_ADDONS_MAIN_FILE), [ 'jquery' ], WC_PRODUCT_ADDONS_VERSION, true);
                ...
                wp_enqueue_script('wcfmu_wcaddons_products_manage_js', $WCFMu->library->js_lib_url.'products/wcfmu-script-wcaddons-products-manage.js', [ 'jquery', 'wcfm_products_manage_js' ], $WCFMu->version, true);
                break;
        }//end switch

    }//end wcaddons_load_scripts_3()
    ...
    function wcaddons_wcfm_products_manage_meta_save_3($new_product_id, $wcfm_products_manage_form_data)
    {
        ...
        if (isset($wcfm_products_manage_form_data['product_addon_name'])) {
            ...
            $addon_description_enable = isset($wcfm_products_manage_form_data['product_addon_description_enable']) ? $wcfm_products_manage_form_data['product_addon_description_enable'] : [];
            ...
            $addon_required           = isset($wcfm_products_manage_form_data['product_addon_required']) ? $wcfm_products_manage_form_data['product_addon_required'] : [];
            ...
            $addon_restrictions       = isset($wcfm_products_manage_form_data['product_addon_restrictions']) ? $wcfm_products_manage_form_data['product_addon_restrictions'] : [];
            $addon_restrictions_type  = $wcfm_products_manage_form_data['product_addon_restrictions_type'];
            $addon_adjust_price       = isset($wcfm_products_manage_form_data['product_addon_adjust_price']) ? $wcfm_products_manage_form_data['product_addon_adjust_price'] : [];
        ...
        uasort($product_addons, [ $this, 'addons_cmp' ]);
    ...
    }//end wcaddons_wcfm_products_manage_meta_save_3()
    ...
    function wcaddons_wcfm_products_manage($product_id)
    {
        ...
        $group_types = apply_filters(
            'wcfm_wcaddon_group_types',
            [
        ...
        <div class="wcfm-container wcaddons">
            <div id="wcfm_products_manage_form_wcaddons_expander" class="wcfm-content">
                ...
                $WCFM->wcfm_fields->wcfm_generate_form_field(
                    apply_filters(
                        'wcfm_product_manage_fields_wcaddons',
                        [
                            '_product_addons'                => [
                                'label'       => __('Add-ons', 'wc-frontend-manager-ultimate'),
                                'type'        => 'multiinput',
                                'class'       => 'wcfm_ele wcaddons',
                                'label_class' => 'wcfm_title wcaddons',
                                'value'       => $_product_addons,
                                'options'     => [
                                    'type'        => [
                                        'label'       => __('Group', 'wc-frontend-manager-ultimate'),
                                        'type'        => 'select',
                                        'options'     => $group_types,
                                        'class'       => 'wcfm-select addon_fields_option wcaddons',
                                        'label_class' => 'wcfm_title wcaddons',
                                    ],
                                    'name'        => [
                                    ...
                                    'required'    => [
                                        'label'       => __('Required fields?', 'wc-frontend-manager-ultimate'),
                                        'type'        => 'checkbox',
                                        'class'       => 'wcfm-checkbox',
                                    ...
                                    'options'     => [
                                        ...
                                        'options'     => [
                                            'label' => [
                                                'label'       => __('Label', 'wc-frontend-manager-ultimate'),
                                                'type'        => 'text',
                                                'class'       => 'wcfm-text',
                                                'label_class' => 'wcfm_title',
                                            ],
                                            'price' => [
                                                'label'       => __('Price', 'wc-frontend-manager-ultimate'),
                                                'type'        => 'text',
                                                'class'       => 'wcfm-text addon_fields addon_price',
                                                'label_class' => 'wcfm_title addon_fields addon_price',
                                            ],
                                            'min'   => [
                                                'label'       => __('Min', 'wc-frontend-manager-ultimate'),
                                                'type'        => 'number',
                                                'class'       => 'wcfm-text addon_fields addon_minmax',
                                                'label_class' => 'wcfm_title addon_fields addon_minmax',
                                            ],
                                            'max'   => [
                                                'label'       => __('Max', 'wc-frontend-manager-ultimate'),
                                                'type'        => 'number',
                                                'class'       => 'wcfm-text addon_fields addon_minmax',
                                                'label_class' => 'wcfm_title addon_fields addon_minmax',
                                            ],
                            ...
                            '_product_addons_exclude_global' => [
                                'label'       => __('Global Addon Exclusion', 'wc-frontend-manager-ultimate'),
                                'type'        => 'checkbox',
                                'class'       => 'wcfm-checkbox',
                                'label_class' => 'wcfm_title checkbox_title',
                                'value'       => 1,
                                'dfvalue'     => $_product_addons_exclude_global,
                                'hints'       => __('Check this to exclude this product from all Global Addons', 'wc-frontend-manager-ultimate'),
                            ],
                        ]
                    )
                );
    ...
    }//end wcaddons_wcfm_products_manage()
        ...
        if (isset($wcfm_products_manage_form_data['_product_addons']) && ! empty($wcfm_products_manage_form_data['_product_addons'])) {
            ...
            if (! empty($_product_addons)) {
                ...
                foreach ($_product_addons as $_product_addon_index => $_product_addon) {
                    ...
                    if (isset($_product_addon['required'])) {
                        $_product_addons[$_product_addon_index]['required'] = 1;
                    } else {
                        $_product_addons[$_product_addon_index]['required'] = 0;
                    }
    ...
    }//end wcaddons_wcfm_products_manage_meta_save()
