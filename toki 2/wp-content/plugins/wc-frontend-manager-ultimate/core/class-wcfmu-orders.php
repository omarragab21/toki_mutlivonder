...
class WCFMu_Orders
{
    ...
    public function __construct()
    {
        ...
        add_filter('wcfm_query_vars', [ &$this, 'wcorder_wcfm_query_vars' ], 20);
        add_filter('wcfm_endpoint_title', [ &$this, 'wcorder_wcfm_endpoint_title' ], 20, 2);
        add_action('init', [ &$this, 'wcorder_wcfm_init' ], 20);

        add_filter('wcfm_menus', [ &$this, 'wcorder_wcfm_menus' ], 20);

        // Order Edit Button
        add_action('after_wcfm_orders_details_items', [ &$this, 'wcorder_order_edit_button' ], 50, 3);

        // Generate Order Edit Form Html
        add_action('wp_ajax_wcfm_edit_order_form_html', [ &$this, 'wcorder_order_edit_form_html' ]);

        // Orders Quick Action - Add Order
        add_action('wcfm_orders_quick_actions', [ &$this, 'wcorder_order_manage' ]);
        add_action('wcfm_after_order_quick_actions', [ &$this, 'wcorder_order_manage' ]);

        // Orders Manage Load WCFMu Scripts
        add_action('wcfm_load_scripts', [ &$this, 'wcorder_load_scripts' ], 30);
        add_action('after_wcfm_load_scripts', [ &$this, 'wcorder_load_scripts' ], 30);

        // Orders Manage Load WCFMu Styles
        add_action('wcfm_load_styles', [ &$this, 'wcorder_load_styles' ], 30);
        add_action('after_wcfm_load_styles', [ &$this, 'wcorder_load_styles' ], 30);

        // Orders Manage Load WCFMu views
        add_action('wcfm_load_views', [ &$this, 'wcorder_load_views' ], 30);

        // Get Customer Address
        add_action('wp_ajax_wcfm_orders_manage_customer_address', [ &$this, 'wcfm_orders_manage_customer_address' ]);

        // Orders Manage Add Customer Ajax Controllers
        add_action('after_wcfm_ajax_controller', [ &$this, 'wcorder_ajax_controller' ], 30);

        // WC Deposit Order Actions
        add_action('wcfm_after_order_itemmeta', [ $this, 'wc_deposit_order_actions' ], 100, 4);

        // WC Deposit Order Actions Handlers
        add_action('wcfm_init', [ $this, 'wc_deposit_order_action_handler' ], 20);
    ...
    }//end wcorder_wcfm_endpoint_title()
    ...
    function wcorder_order_edit_button($order_id, $order, $line_items)
    {
        ...
        if (in_array($order_status, apply_filters('wcfm_edit_order_block_status', [ 'failed', 'cancelled', 'refunded', 'processing', 'completed' ]))) {
            return;
        }
    ...
    function wcorder_order_edit_form_html()
    {
        ...
        if (isset($_POST['order_id']) && ! empty($_POST['order_id'])) {
            $WCFMu->template->get_template('orders/wcfmu-view-orders-edit-popup.php', [ 'order_id' => sanitize_text_field($_POST['order_id']) ]);
        }

        die;

    }//end wcorder_order_edit_form_html()
    ...
    public function wcorder_load_scripts($end_point)
    {
        ...
        switch ($end_point) {
            case 'wcfm-orders-manage':
                ...
                wp_enqueue_script('wcfm_orders_manage_js', $WCFMu->library->js_lib_url.'orders/wcfmu-script-orders-manage.js', [ 'jquery' ], $WCFMu->version, true);
    ...
    public function wcorder_load_styles($end_point)
    {
        ...
        switch ($end_point) {
            case 'wcfm-orders-manage':
                wp_enqueue_style('wcfm_orders_manage_css', $WCFMu->library->css_lib_url.'orders/wcfmu-style-orders-manage.css', [], $WCFMu->version);
                break;
        }

    }//end wcorder_load_styles()
    ...
    }//end wcfm_orders_manage_customer_address()
        ...
        if (class_exists('WC_Deposits_Order_Item_Manager')) {
            if (WC_Deposits_Order_Item_Manager::is_deposit($item)) {
                ...
                if ($payment_plan = WC_Deposits_Order_Item_Manager::get_payment_plan($item)) {
                    ...
                    } else if ($remaining_balance_paid) {
                        printf(__('The remaining balance of %s (plus tax) for this item was paid offline.', 'woocommerce-deposits'), wc_price($remaining, [ 'currency' => $currency ]));
                        echo ' <a href="'.esc_url(wp_nonce_url(add_query_arg([ 'mark_deposit_unpaid' => $item_id ]), 'mark_deposit_unpaid', 'mark_deposit_unpaid_nonce')).'" class="button button-small">'.sprintf(__('Unmark as Paid', 'woocommerce-deposits')).'</a>';
                    } else if (! $this->has_deposit_without_future_payments($order_id)) {
                        ?>
                        <a href="
                        <?php
                        echo esc_url(
                            wp_nonce_url(
                                add_query_arg(
                                    [
                                        'post'                      => $order_id,
                                        'invoice_remaining_balance' => $item_id,
                                    ],
                                    get_wcfm_view_order_url($order_id)
                                ),
                                'invoice_remaining_balance',
                                'invoice_remaining_balance_nonce'
                            )
                        );
                        ?>
                                    " class="button button-small"><?php _e('Invoice Remaining Balance', 'woocommerce-deposits'); ?></a>
                        <a href="
                        <?php
                        echo esc_url(
                            wp_nonce_url(
                                add_query_arg(
                                    [
                                        'post'                    => $order_id,
                                        'mark_deposit_fully_paid' => $item_id,
                                    ],
                                    get_wcfm_view_order_url($order_id)
                                ),
                                'mark_deposit_fully_paid',
                                'mark_deposit_fully_paid_nonce'
                            )
                        );
                        ?>
                                    " class="button button-small"><?php printf(__('Mark Paid (offline)', 'woocommerce-deposits')); ?></a>
                        <?php
                    }//end if
                }//end if
            } else if (! empty($item['original_order_id'])) {
     ...
     *
     * @param  integer|WC_Order $order Order ID or object.
     * @return boolean
     */
    ...
    }//end has_deposit_without_future_payments()
    ...
    }//end wc_deposit_order_action_handler()
    ...
    public static function create_order($payment_date, $original_order_id, $payment_number, $item, $status='')
    {
        ...
        $item_id = $new_order->add_product(
            ...
            // cost before discount (for line quantity, not just unit)
                    'total'        => $item['total'],
            // item cost (after discount) (for line quantity, not just unit)
                    'subtotal_tax' => 0,
            // calculated within (WC_Abstract_Order) $new_order->calculate_totals
                    'tax'          => 0,
            // calculated within (WC_Abstract_Order) $new_order->calculate_totals
                ],
            ]
        ...
        wc_update_order_item($item_id, [ 'order_item_name' => sprintf(__('Payment #%1$d for %2$s', 'woocommerce-deposits'), $payment_number, $item['product']->get_title()) ]);
