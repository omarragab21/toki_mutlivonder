...
class WCFMu_Support
{
    ...
    function wcfm_support_menus( $menus )
    {
        ...
        $menus = array_slice($menus, 0, 3, true) +
                                                array(
                                                    'wcfm-support' => array(
                                                        'label' => __('Support', 'wc-frontend-manager-ultimate'),
    ...
    function wcfm_support_order_action( $actions, $order )
    {
        ...
        if ( in_array($order_status, apply_filters('wcfm_support_disable_order_status', array( 'failed', 'cancelled', 'refunded', 'pending', 'on-hold', 'request', 'proposal', 'proposal-sent', 'proposal-expired', 'proposal-rejected', 'proposal-canceled', 'proposal-accepted' ))) ) {
            return $actions;
        }

        $order_id                       = $order->get_id();
        $actions['wcfm-support-action'] = array(
            'name' => __('Support', 'wc-frontend-manager-ultimate'),
            'url'  => '#' . $order_id,
        );
        return $actions;
    }
