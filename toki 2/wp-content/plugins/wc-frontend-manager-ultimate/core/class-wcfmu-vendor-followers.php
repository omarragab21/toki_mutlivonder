        ...
        if (! empty($vendor_followers)) {
            foreach ($vendor_followers as $vendor_follower) {
                if ($vendor_follower->follower_email) {
                    ...
                    $notification_mail_body   = '<br/>'.__('Hi', 'wc-frontend-manager').' {follower_name}'.',<br/><br/>'.__('A new product submitted by ', 'wc-frontend-manager-ultimate').' <b>{vendor_store}</b>'.'<br/><br/>'.sprintf(__('Check the product %1$s{product_title}%2$s.', 'wc-frontend-manager-ultimate'), '<a href="{product_url}">', '</a>').'<br /><br/>'.__('Thank You', 'wc-frontend-manager').'<br/><br/>';
