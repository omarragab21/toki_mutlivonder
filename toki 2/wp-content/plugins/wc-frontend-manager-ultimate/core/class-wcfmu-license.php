...
class WCFMu_License
{
    ...
    public $upgrade_url = WCFMu_SERVER_URL;
    ...
    public function __construct()
    {
        ...
        if (is_admin()) {
            ...
            $this->license_plugin_name = 'wc-frontend-manager-ultimate/wc_frontend_manager_ultimate.php';
            // same as plugin slug. if a theme use a theme name like 'twentyeleven'
            $this->license_product_id = get_option($this->license_product_id_key);
            // Software Title
            $this->license_renew_license_url = WCFMu_SERVER_URL.'/my-account';
            // URL to renew a license
            $this->license_instance_id = get_option($this->license_instance_key);
            // Instance ID (unique to each blog activation)
            $this->license_domain = str_replace('http://', '', str_replace('https://', '', site_url()));
            // blog domain name
            $this->license_software_version = $WCFMu->version;
            // The software version
            $this->license_plugin_or_theme = 'plugin';
            // 'theme' or 'plugin'
            $this->license_plugin_or_theme_mode = 'paid';
            // 'paid' or 'free'
    ...
    public function add_menu()
    {
        ...
        if (empty($GLOBALS['admin_page_hooks']['wcfm_licenses'])) {
            if (! class_exists('WCFM_License')) {
                include_once $WCFMu->plugin_path.'core/license/admin/class-wcfm-license.php';
                new WCFM_License('wcfmu_license');
            }
        }

        add_filter('wcfm_license_tabs', [ &$this, 'license_new_tab' ], 10, 1);
        add_action('settings_page_'.str_replace('-', '_', esc_attr($WCFMu->token)).'_license_tab_init', [ &$this, 'license_tab_init' ], 10, 1);
    ...
    function load_class($class_name='')
    {
        global $WCFMu;
        if ('' != $class_name) {
            include_once $WCFMu->plugin_path.'core/license/classes/class-'.esc_attr($WCFMu->token).'-'.esc_attr($class_name).'.php';
        } //end if
    ...
    function load_class_admin($class_name='')
    {
        global $WCFMu;
        if ('' != $class_name) {
            include_once $WCFMu->plugin_path.'core/license/admin/class-'.esc_attr($WCFMu->token).'-'.esc_attr($class_name).'.php';
        } //end if

    }//end load_class_admin()
    ...
    public function license_key_deactivation()
    {
        ...
        if ($activation_status == 'Activated' && $api_key != '' && $api_email != '') {
            $this->api_manager_license_key->deactivate($args);
            // reset license key activation
        }

    }//end license_key_deactivation()
    ...
    public static function license_inactive_notice()
    {
        ...
        ?>
        <?php
        if (isset($_GET['page']) && 'api_manager_license_dashboard' == $_GET['page']) {
            return;
        }
        ?>
        <div id="message" class="error settings-error notice is-dismissible">
            <p><?php printf(__('The WCFM - Ultimate License Key has not been activated, so the plugin is inactive! %1$sClick here%2$s to activate the license key and the plugin.', 'wc-frontend-manager-ultimate'), '<a href="'.esc_url(admin_url('admin.php?page=wcfm-license&tab='.str_replace('-', '_', esc_attr($WCFMu->token)).'_license')).'">', '</a>'); ?></p>
            <button class="notice-dismiss" type="button"><span class="screen-reader-text">Dismiss this notice.</span></button>
        </div>
        <?php

    }//end license_inactive_notice()
