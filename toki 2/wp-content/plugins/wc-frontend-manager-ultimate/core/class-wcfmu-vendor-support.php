...
class WCFMu_Vendor_Support
{
    ...
    public function __construct()
    {
        ...
        add_filter('wcfm_capability_settings_fields_product_types', [ &$this, 'wcfmcap_product_types' ], 10, 3);

        // Associate Store For Imported Products
        if ($WCFMu->is_marketplace && ( $WCFMu->is_marketplace == 'wcfmmarketplace' )) {
            add_filter('woocommerce_csv_product_import_mapping_options', [ &$this, 'wcfmu_product_import_vendor_options' ], 100, 2);
            add_action('woocommerce_product_import_inserted_product_object', [ &$this, 'wcfmu_product_import_vendor_association' ], 100, 2);
        }

        if (! is_admin()) {
            add_filter('get_edit_post_link', [ &$this, 'wfm_edit_post_link' ], 100, 3);
        }

        if ($is_marketplace = wcfm_is_marketplace()) {
            // WCFMu WCMp Endpoint Edit - 2.5.3
            add_filter('wcfm_endpoints_slug', [ &$this, 'wcmarketplace_wcfmu_endpoints_slug' ]);

            if (apply_filters('wcfm_is_allow_commission_manage', true)) {
                if ($is_marketplace == 'wcvendors') {
                    if (wcfm_is_wcvpro()) {
                        if (! wcfm_is_vendor()) {
                            add_action('end_wcfm_products_manage', [ &$this, 'wcvendorspro_product_commission' ], 499);

                            // Commision Save
                            add_action('after_wcfm_products_manage_meta_save', [ &$this, 'wcvendorspro_product_commission_save' ], 500, 2);
                        }
                    }
                }
            }
        ...
        add_filter('wcfm_navigation_url', [ &$this, 'wcfmu_get_navigation_url' ], 10, 2);
    ...
    function wcfmcap_product_types($product_types, $handler='wcfm_capability_options', $wcfm_capability_options=[])
    {
        ...
        $product_types['virtual']      = [
            'label'       => __('Virtual', 'wc-frontend-manager-ultimate'),
            'name'        => $handler.'[virtual]',
            'type'        => 'checkboxoffon',
        ...
        $product_types['downloadable'] = [
            ...
            'name'        => $handler.'[downloadable]',
            'type'        => 'checkboxoffon',
            'class'       => 'wcfm-checkbox wcfm_ele',
            'value'       => 'yes',
            'label_class' => 'wcfm_title checkbox_title',
            'dfvalue'     => $downloadable,
        ];
    ...
    }//end wcfmu_product_import_vendor_options()
    ...
    function wcfmu_product_import_vendor_association($product_obj, $data)
    {
        ...
        if (isset($data['pwb-brand'])) {
            ...
            foreach ($brands as $brand) {
                wp_set_object_terms($new_product_id, $brand, 'pwb-brand', true);
            }
        }

    }//end wcfmu_product_import_vendor_association()
    ...
    }//end wcmarketplace_wcfmu_endpoints_slug()
    ...
    function wcvendorspro_product_commission($product_id)
    {
        ...
        remove_action('end_wcfm_products_manage', [ &$WCFM->wcfm_vendor_support, 'wcvendors_product_commission' ], 500);
        ...
        <div class="wcfm-container simple variable external grouped booking">
            <div id="wcfm_products_manage_form_commission_expander" class="wcfm-content">
                ...
                $WCFM->wcfm_fields->wcfm_generate_form_field(
                    apply_filters(
                        'product_manage_fields_commission',
                        [
                            'wcv_commission_type'    => [
                                'label'       => __('Commission Type', 'wc-frontend-manager-ultimate'),
                                'type'        => 'select',
                                ...
                                'value'       => $wcv_commission_type,
                            ],
                            'wcv_commission_percent' => [
                                'label'       => __('Commission(%)', 'wc-frontend-manager-ultimate'),
                                'type'        => 'number',
                                'attributes'  => [
                                    'min'   => '',
                                    'steps' => 1,
                                ...
                                'label_class' => 'wcfm_title wcfm_ele simple variable external grouped booking',
                                'value'       => $wcv_commission_percent,
                            ],
                            'wcv_commission_amount'  => [
                                'label'       => __('Commission Fixed', 'wc-frontend-manager-ultimate'),
                                'type'        => 'number',
                                'attributes'  => [
                                    'min'   => '',
                                    'steps' => 1,
                                ...
                                'class'       => 'wcfm-text wcfm_ele simple variable external grouped booking',
                                'label_class' => 'wcfm_title wcfm_ele simple variable external grouped booking',
                                'value'       => $wcv_commission_amount,
                            ],
                            'wcv_commission_fee'     => [
                                'label'       => __('Commission Fee', 'wc-frontend-manager-ultimate'),
                                'type'        => 'number',
                                'attributes'  => [
                                    'min'   => '',
                                    'steps' => 1,
    ...
    }//end wcvendorspro_product_commission()
    ...
    }//end wcvendorspro_product_commission_save()
    ...
    }//end get_wcfm_verification_badge()
    ...
    }//end wcfm_show_verified_seller_badge()
