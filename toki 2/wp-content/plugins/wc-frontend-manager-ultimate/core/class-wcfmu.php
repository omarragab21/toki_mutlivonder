...
class WCFMu
{
    ...
    public function __construct($file)
    {
        ...
        add_action('wcfm_init', [ &$this, 'init_wcfmu' ], 12);

        add_action('wp', [ &$this, 'wcfmu_init_after_wp' ], 500);

        add_action('plugins_loaded', [ $this, 'wca_includes' ], 10);

        add_filter('wcfm_modules', [ &$this, 'get_wcfmu_modules' ], 20);
    ...
    function wca_includes()
    {
        ...
        if (WCFMu_Dependencies::wcfm_wc_pdf_voucher_active_check()) {
            add_filter('woo_vou_edit_vendor_role', [ &$this, 'wcfm_woo_vou_edit_vendor_role' ], 180);
        }
    ...
    function init_wcfmu()
    {
        ...
        if (! is_admin() || defined('DOING_AJAX') || defined('WCFM_REST_API_CALL')) {
            if ($this->is_marketplace) {
                if (wcfm_is_vendor()) {
                    ...
                    if ($this->is_marketplace == 'wcvendors') {
                        $this->wcfmu_marketplace = new WCFMu_WCVendors();
                    }

                    if ($this->is_marketplace == 'wcpvendors') {
                        $this->wcfmu_marketplace = new WCFMu_WCPVendors();
                    }

                    if ($this->is_marketplace == 'wcmarketplace') {
                        $this->wcfmu_marketplace = new WCFMu_WCMarketplace();
                    }

                    if ($this->is_marketplace == 'dokan') {
                        $this->wcfmu_marketplace = new WCFMu_Dokan();
                    }

                    if ($this->is_marketplace == 'wcfmmarketplace') {
                        $this->wcfmu_marketplace = new WCFMu_Marketplace();
                    }
                }//end if
            }//end if
        }//end if
        ...
        if ($this->is_marketplace && in_array($this->is_marketplace, [ 'dokan' ])) {
        ...
        // Check WC Fancy Product Designer - 5.5.0
    ...
    }//end wcfm_woo_vou_edit_vendor_role()
    ...
    public function load_class($class_name='')
    {
        if ('' != $class_name && '' != $this->token) {
            include_once 'class-'.esc_attr($this->token).'-'.esc_attr($class_name).'.php';
        } //end if
    ...
    function get_wcfmu_modules($wcfm_modules)
    {
        ...
        $wcfmu_modules = [
            'product_import'      => [
                'label'  => __('Product Import', 'wc-frontend-manager'),
                'notice' => true,
            ],
            'bulk_stock_manager'  => [ 'label' => __('Bulk Stock Manager', 'wc-frontend-manager') ],
            'shipment_tracking'   => [ 'label' => __('Shipment Tracking', 'wc-frontend-manager-ultimate') ],
            'support'             => [ 'label' => __('Support Ticket', 'wc-frontend-manager-ultimate') ],
            'vendor_invoice'      => [ 'label' => __('Vendor Invoice', 'wc-frontend-manager-ultimate') ],
            'vendor_vacation'     => [ 'label' => __('Vendor Vacation', 'wc-frontend-manager-ultimate') ],
            'vendor_verification' => [ 'label' => __('Vendor Verification', 'wc-frontend-manager-ultimate') ],
            'vendor_badges'       => [ 'label' => __('Vendor Badges', 'wc-frontend-manager-ultimate') ],
            'vendor_followers'    => [ 'label' => __('Following / Followers', 'wc-frontend-manager-ultimate') ],
            'chatbox'             => [ 'label' => __('Chat Box', 'wc-frontend-manager-ultimate') ],
        ...
        if ($WCFMu->is_marketplace && ( $WCFMu->is_marketplace == 'wcfmmarketplace' )) {
            $wcfmu_modules['shipstation']          = [ 'label' => __('ShipStation', 'wc-frontend-manager-ultimate') ];
            $wcfmu_modules['facebook_marketplace'] = [ 'label' => __('Facebook for Marketplace', 'wc-frontend-manager-ultimate') ];
        }

        $wcfm_modules = array_merge($wcfm_modules, $wcfmu_modules);
        return $wcfm_modules;

    }//end get_wcfmu_modules()
