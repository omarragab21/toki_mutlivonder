			...
			formatDate: function(format, date, settings) {
				...
				* - ignore rest of string
					...
					if (inst.inline) {
						inst.drawDate = plugin._checkMinMax(
							plugin.newDate(
								...
								inst.get( 'defaultDate' ) || plugin.today()
							),
							inst
						);
				...
				_autoSize: function(target, inst) {
					if (inst.options.autoSize && ! inst.inline) {
						...
						if (dateFormat.match( /[DM]/ )) {
							...
							date.setMonth(
								findMax(
									inst.options[dateFormat.match( /MM/ ) ? // Longest month
                                'monthNames' : 'monthNamesShort']
								)
							);
							date.setDate(
								findMax(
									inst.options[dateFormat.match( /DD/ ) ? // Longest day
				...
				_enablePlugin: function(target) {
					...
					this._disabled = $.map(
						this._disabled,
						function(value) { return (value == target[0] ? null : value); }
					); // Delete entry
				},
				...
				_extractDates: function(inst, datesText) {
					...
					datesText    = datesText.split(
						inst.options.multiSelect ? inst.options.multiSeparator :
				...
				_showMonthPlugin: function(target, year, month, day) {
					...
					(inst.drawDate.getFullYear() != year || inst.drawDate.getMonth() + 1 != month))) {
						...
						var show      = this._checkMinMax(
							(year != null ?
							plugin.newDate( year, month, 1 ) : plugin.today()),
							inst
						);
						inst.drawDate = plugin.newDate(
							show.getFullYear(),
				...
				_generateContent: function(target, inst) {
					...
					find( 'select.' + this._anyYearClass ).click(
						function() {
					...
					find( 'input.' + self._monthYearClass ).change(
						function() {
							try {
					...
					picker.find( '.' + inst.options.renderer.commandClass ).click(
						function() {
					...
					$( 'body' ).append( picker );
					var width = 0;
					picker.find( inst.options.renderer.monthSelector ).each(
						function() {
