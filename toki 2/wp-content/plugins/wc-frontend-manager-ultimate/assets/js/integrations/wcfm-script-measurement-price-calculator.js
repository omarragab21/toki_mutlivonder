jQuery( document ).ready(
	function ($) {
		...
		$( '#calculator-settings' ).find( 'label' ).each(
			function () {
				var content = $( this ).html();
				$( this ).replaceWith( '<span class="wcfm_title"><strong>' + content + '</strong></span>' );
			}
		);
		$( '#calculator-settings' ).find( '.description' ).each(
			function () {
				var content = $( this ).html();
				$( this ).replaceWith( '<p class="description wcfm_page_options_desc">' + content + '</p>' );
			}
		);
		$( '#calculator-settings' ).find( 'input[type="text"]' ).addClass( 'wcfm-text' );
		$( '#calculator-settings' ).find( 'input[type="number"]' ).addClass( 'wcfm-text wcfm_small_ele' );
		$( '#calculator-settings' ).find( 'select' ).addClass( 'wcfm-select' );
		$( '#calculator-settings' ).find( 'input[type="checkbox"]' ).addClass( 'wcfm-checkbox' );
		...
		$( '#calculator-settings' ).find( 'input[type="checkbox"]' ).change(
			function () {
				setTimeout(
					function () {
						resetCollapsHeight( jQuery( '#measurement_product_data' ) );
					},
					100
				);
			}
		...
		$( '#calculator-settings' ).find( 'input[type="radio"]' ).click(
			function () {
				setTimeout(
					function () {
						resetCollapsHeight( jQuery( '#measurement_product_data' ) );
		...
		function addInventoryPerUnitLabel(unitLabel)
		{
			...
			// first clean the label in case we've already modified it
		...
		$( '._measurement_pricing' ).change(
			function () {
				...
				if ('_measurement_' + $( '#_measurement_price_calculator' ).val() + '_pricing' === $( this ).attr( 'id' )) {
					if ($( this ).is( ':checked' )) {
						$( this ).closest( 'div.measurement_fields' ).find( '._measurement_pricing_fields' ).show();
						// display the pricing fields
					} else {
			...
			function () {
				...
				if ('_measurement_' + $( '#_measurement_price_calculator' ).val() + '_pricing_calculator_enabled' === $( this ).attr( 'id' )) {
					if ($( this ).closest( 'div.measurement_fields' ).find( '._measurement_pricing' ).is( ':checked' ) && $( this ).is( ':checked' )) {
						addPricingPerUnitLabel( $( this ).closest( 'div' ).find( '._measurement_pricing_unit option:selected' ).text() );
						// update the product label like 'Price ($)' to 'Price ($/sq ft)'
						$( this ).closest( 'div.measurement_fields' ).find( '._measurement_editable' ).attr( 'checked','checked' ).attr( 'disabled','disabled' );
						// force the 'editable' field to be enabled, for consistency since this product is now customizable by the customer
						$( this ).closest( 'div.measurement_fields' ).find( '._measurement_pricing_calculator_fields' ).each(
							function (index, el) {
						...
						if ($( this ).closest( 'div.measurement_fields' ).find( '._measurement_editable' ).is( ':disabled' )) {
							$( this ).closest( 'div' ).find( '._measurement_editable' ).removeAttr( 'disabled' ); }
		...
		$( '#measurement_product_data' ).on(
			'change',
			'._measurement_accepted_input',
			function () {
				var $this             = $( this ),
		...
		.on(
			'click',
			'._measurement_input_attributes label',
			function () {
		...
		$( 'select._measurement_accepted_input' ).on(
			'change',
			function () {
		...
		$( '._measurement_pricing_weight_enabled' ).change(
			function () {
				...
				if ( ! wc_price_calculator_admin_params.woocommerce_weight_unit) {
					return; }
		...
		$( '._measurement_pricing_unit' ).change(
			function () {
				$( '._measurement_pricing' ).change();
			}
		);
		...
		$( '._measurement_dimension' ).change(
			function () {
				$( '._measurement_dimension' ).each(
					function (index,el) {
						var id = $( el ).attr( 'id' );
						if ($( '#' + id ).is( ':checked' )) {
							$( '#' + id + '_fields' ).show(); } else {
											$( '#' + id + '_fields' ).hide(); }
					}
				);
			}
		...
		$( 'button.wc-calculator-pricing-table-add-rule' ).click(
			function () {
				var index = $( 'table.wc-calculator-pricing-table tbody tr' ).length;

				$( 'table.wc-calculator-pricing-table > tbody' ).append(
					'<tr class="wc-calculator-pricing-rule"><td class="check-column"><input type="checkbox" class="wcfm-checkbox" name="select" /></td>\
			<td class="wc-calculator-pricing-rule-range"><input type="text" class="wcfm-text" name="_wc_measurement_pricing_rule_range_start[' + index + ']" value="" /> - <input type="text" class="wcfm-text" name="_wc_measurement_pricing_rule_range_end[' + index + ']" value="" /></td>\
			<td><input type="text" class="wcfm-text" name="_wc_measurement_pricing_rule_regular_price[' + index + ']" value="" /></td>\
			<td><input type="text" class="wcfm-text" name="_wc_measurement_pricing_rule_sale_price[' + index + ']" value="" /></td></tr>'
				);
		...
		$( 'button.wc-calculator-pricing-table-delete-rules' ).click(
			function () {
				$( 'table.wc-calculator-pricing-table td.check-column input:checked' ).each(
					function () {
						$( this ).closest( 'tr.wc-calculator-pricing-rule' ).fadeOut(
							'400',
							function () {
								$( this ).remove();
		...
		{
			var loop = 0;
			$( 'table.wc-calculator-pricing-table tbody tr' ).each(
				function ( index, row ) {
					$( 'input', row ).each(
						function ( i, el ) {
							var t = jQuery( el );
