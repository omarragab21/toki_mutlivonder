jQuery( document ).ready(
	function ($) {
		...
		$( '#redq_inventory_products' ).find( 'select:not(.wcfm_half_ele)' ).each(
			function () {
		...
		setTimeout(
			function () {
				$( '#redq_inventory_products' ).find( '.add_multi_input_block' ).click(
					function () {
						$( '#redq_inventory_products' ).find( '.multi_input_block:last' ).find( 'select:not(.wcfm_half_ele)' ).each(
							function () {
								$( this ).select2();
							}
						);
				...
				$( '#redq_inventory_products' ).find( '.redq_rental_availability' ).find( '.add_multi_input_block' ).click(
					function () {
		...
		$( '#redq_inventory_products' ).children( '.multi_input_block' ).each(
			function () {
		...
		// Inventory Collapser
		$( '#redq_inventory_products' ).children( '.multi_input_block' ).children( '.add_multi_input_block' ).click(
			function () {
				$( '#redq_inventory_products' ).children( '.multi_input_block' ).children( '.variations_collapser' ).each(
					function () {
						$( this ).off( 'click' ).on(
							'click',
							function () {
				...
				$( '#redq_inventory_products' ).children( '.multi_input_block:last' ).find( 'select:not(.wcfm_half_ele)' ).each(
					function () {
						$( this ).val( '' ).select2( { placeholder: $( this ).data( 'placeholder' ) } );
					}
				);
			}
		);
		...
		$( '.redq_rental_settings_type' ).each(
			function () {
				$( this ).change(
					function () {
