jQuery(document).ready(function ($) {
	...
	if (current_date == today) {
		...
		var current_time_locale = d
			.toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" })
			.toLowerCase();
		var indicator_top = Math.round((calendar_h / (60 * 24)) * current_time);
		$(".bytime.appointments").append(
			'<div class="time_indicator tips" title="' +
				current_time_locale +
				'"></div>'
		);
	...
	function set_indicator() {
		...
		var current_time_locale_updated = dt
			.toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" })
			.toLowerCase();
		var indicator_top = Math.round((calendar_h / (60 * 24)) * current_time);
		$(".time_indicator").css({ top: indicator_top });
	...
	$(".event_card").each(function () {
		$(this).click(function (event) {
			...
			$card = $(this);
			$data = $card.data();
			jQuery("#aptProduct").html($data.product_title ? $data.product_title: $data.event_name);
			jQuery("#aptStart").html($data.when);
			jQuery("#aptDuration").html($data.duration);
			jQuery("#aptQuantity").html($data.event_qty);
			jQuery("#aptStaff").html($data.staff_name);
			jQuery("#aptCustName").html($data.customer_name);
			jQuery("#aptCustEmail").html($data.customer_email);
			jQuery("#aptCustPhone").html($data.customer_phone);
			jQuery("#aptLink").attr("href", $card.find("a").attr("href"));
