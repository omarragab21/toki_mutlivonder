...
class WCFMu_Booking_Controller
{
    ...
    public function processing()
    {
        ...
        $wc_get_booking_status_name = apply_filters(
            'wcfmu_bookings_menus',
            [
        ...
        if (isset($_POST['booking_status']) && ! empty($_POST['booking_status'])) {
            $args['post_status'] = $_POST['booking_status'];
        }
        ...
        if (! empty($wcfm_bookings_array)) {
            ...
            foreach ($wcfm_bookings_array as $wcfm_bookings_single) {
                ...
                if (in_array([ 'was-in-cart', 'in-cart' ], $status)) {
                    continue;
                }
                ...
                $product_label = '';
                if ($product) {
                    $product_label = '<a target="_blank" href="'.get_wcfm_edit_product_url(( is_callable([ $product, 'get_id' ]) ? $product->get_id() : $product->id )).'">'.$product->get_title().'</a>';
                ...
                $wcfm_bookings_json_arr[$index][] = wc_price($final_amt, [ 'currency' => $currency ]);
                ...
                if (current_user_can('manage_bookings_settings') || current_user_can('manage_bookings')) {
                    if (in_array($booking->get_status(), [ 'pending-confirmation' ])) {
                        $actions = '<a class="wcfm_tych_booking_mark_confirm wcfm-action-icon" href="#" data-bookingid="'.$wcfm_bookings_single->ID.'"><span class="wcfmfa fa-check-circle text_tip" data-tip="'.esc_attr__('Mark as Confirmed', 'wc-frontend-manager').'"></span></a>';
                    }
                }
        ...
        if (! empty($wcfm_bookings_json_arr)) {
            $wcfm_bookings_json .= json_encode($wcfm_bookings_json_arr);
        } else {
            $wcfm_bookings_json .= '[]';
        }
