...
class WCFMu_Orders_Edit_Controller
{
    ...
    public function processing()
    {
        ...
        if (isset($wcfm_orders_edit_form_data['wcfm_order_edit_id']) && ! empty($wcfm_orders_edit_form_data['wcfm_order_edit_id'])) {
            ...
            // Add Order Note
            if (wcfm_is_vendor()) {
                $wcfm_messages = sprintf(__('<b>%1$s</b> order updated by <b>%2$s</b>.', 'wc-frontend-manager-ultimate'), '#<a target="_blank" class="wcfm_dashboard_item_title" href="'.get_wcfm_view_order_url($order_id).'">'.$order_id.'</a>', $WCFM->wcfm_vendor_support->wcfm_get_vendor_store_by_vendor($vendor_id));
            ...
            if (! $has_error) {
                echo '{"status": true, "message": "'.$wcfm_orders_manage_messages['order_updated'].'", "redirect": "'.get_wcfm_view_order_url($new_order_id).'"}';
            } else {
