...
class WCFMu_WCAppointments_Controller
{
    ...
    public function processing()
    {
        ...
        $wc_get_appointment_status_name = [
        ...
        if (isset($_POST['appointment_status']) && ! empty($_POST['appointment_status']) && ( $_POST['appointment_status'] != 'all' )) {
            $args['post_status'] = $_POST['appointment_status'];
        }
        ...
        } else if (! empty($_POST['appointment_filter']) && empty($_POST['appointment_staff_filter'])) {
            $args['meta_query'] = [
                [
                    'key'   => '_appointment_product_id',
                    'value' => absint($_POST['appointment_filter']),
        ...
        if (! empty($wcfm_appointments_array)) {
            ...
            foreach ($wcfm_appointments_array as $wcfm_appointments_single) {
                ...
                if ($the_appointment->has_status([ 'was-in-cart', 'in-cart' ])) {
                    continue;
                }
                ...
                if (current_user_can('manage_appointments')) {
                    if (in_array($the_appointment->get_status(), [ 'pending-confirmation' ])) {
                        $actions = '<a class="wcfm_appointment_mark_confirm wcfm-action-icon" href="#" data-appointmentid="'.$wcfm_appointments_single->ID.'"><span class="wcfmfa fa-check-circle text_tip" data-tip="'.esc_attr__('Mark as Confirmed', 'wc-frontend-manager-ultimate').'"></span></a>';
                    }
                }
        ...
        if (! empty($wcfm_appointments_json_arr)) {
            $wcfm_appointments_json .= json_encode($wcfm_appointments_json_arr);
        } else {
            $wcfm_appointments_json .= '[]';
        }
