...
class WCFM_Bookings_Calendar
{
    ...
    protected function get_events_data_for_days($days, $events)
    {
        ...
        foreach ($events as $event) {
            if ('WC_Booking' === get_class($event)) {
                ...
                // Extract global availability data.
                $availability = $event;
                ...
                foreach ($days as $day) {
                    ...
                    $detail_href      = '#';
                    // admin_url( 'edit.php?post_type=wc_booking&page=wc_bookings_settings' );
    ...
    public function list_bookings($day, $month, $year)
    {
        ...
        foreach ($this->events as $event) {
            $event_classes = [ 'wc-bookings-event-link' ];
            $title         = '';
            $id            = '';
            $style         = '';
    ...
    private function get_available_minutes_for_calendar_day($day)
    {
        ...
        if (0 < count($rules)) {
            ...
            usort($global_rules, [ 'WC_Product_Booking_Rule_Manager', 'sort_rules_callback' ]);
            return WC_Product_Booking_Rule_Manager::get_minutes_from_rules($global_rules, $day_start, range(0, 1439));
        } else {
            return range(0, 1439);
        }

    }//end get_available_minutes_for_calendar_day()
    ...
    public function list_bookings_for_day()
    {
        ...
        foreach ($this->events as $index => $event) {
            ...
            $css_classes = [ 'daily_view_booking' ];
    ...
    public function list_global_availability_for_day()
    {
        ...
        $color_style           = ['background' => '#bdbdbd2b'];
        $classes               = [ 'daily_view_global_availabiltiy' ];
        foreach ($not_available_ranges as $range) {
            echo wp_kses_post($this->render_li_element($range, $color_style, $classes));
        }

    }//end list_global_availability_for_day()
     ...
     * @return string
     */
    protected function render_li_element($data, $color_style, $classes=[])
    {
        $li_attrs = ['style' => $color_style];
    ...
    }//end split_minutes_array_into_ranges()
    ...
    protected function get_booking_data($event, $check_date=null)
    {
        ...
        $booking_customer = '';
        $booking_id       = is_callable([ $event, 'get_id' ]) ? $event->get_id() : '';
        $booking_resource = '';
        $booking_persons  = [];
        $event_url        = '';
        ...
        if ('WC_Booking' === get_class($event)) {
            ...
            if ($product && in_array($product->get_duration_unit(), [ 'hour', 'minute' ], true)) {
     ...
     *
     * @param  boolean $include_resources If true, each product's resources will be included.
