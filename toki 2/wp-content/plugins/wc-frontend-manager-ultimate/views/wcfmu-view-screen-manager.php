...
if (isset($_POST['screen'])) {
    ...
    $screen_manager_options   = [];
    if ($screen) {
        switch ($screen) {
            case 'product':
                $screen_manager_options = [
                    1  => __('Image', 'wc-frontend-manager-ultimate'),
            ...
            case 'coupon':
                $screen_manager_options = [
                    0 => __('Code', 'wc-frontend-manager-ultimate'),
            ...
            case 'order':
                $screen_manager_options = [
                    0  => __('Status', 'wc-frontend-manager-ultimate'),
            ...
            case 'listing':
                $screen_manager_options = [
                    0  => __('Listing', 'wc-frontend-manager'),
        ...
        if (isset($wcfm_screen_manager[$screen])) {
            $wcfm_screen_manager_data = $wcfm_screen_manager[$screen];
        }
          ...
          <tbody>
            <?php foreach ($screen_manager_options as $screen_manager_option_index => $screen_manager_option) { ?>
                        <tr>
                            ...
                            <?php
                            if (in_array($screen_manager_option_index, $wcfm_screen_manager_data['admin'])) {
                                echo 'checked="checked"';
                            }
                            ?>
                             name="wcfm_screen_manager[<?php echo $screen; ?>][admin][<?php echo $screen_manager_option_index; ?>]" value="<?php echo $screen_manager_option_index; ?>" /></td>
                            <?php if (wcfm_is_marketplace()) { ?>
                                <td><input type="checkbox" 
                                <?php
                                if (in_array($screen_manager_option_index, $wcfm_screen_manager_data['vendor'])) {
                                    echo 'checked="checked"';
                                }
                                ?>
                                 name="wcfm_screen_manager[<?php echo $screen; ?>][vendor][<?php echo $screen_manager_option_index; ?>]" value="<?php echo $screen_manager_option_index; ?>" /></td>
                            <?php } ?>
                        </tr>
                <?php
            }//end foreach
