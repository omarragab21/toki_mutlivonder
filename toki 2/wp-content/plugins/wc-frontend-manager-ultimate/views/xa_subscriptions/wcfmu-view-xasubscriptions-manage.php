...
} else if ($subscripton_post->post_status == 'wc-expired') {
    $statuses = apply_filters(
        'wcfmu_subscriptions_status',
        [
...
<div class="collapse wcfm-collapse" id="wcfm_subscription_details">
  ...
  <div class="wcfm-page-headig">
                                    ...
                                    <td>
                                        ...
                                        if (apply_filters('wcfm_allow_customer_billing_details', true)) {
                                            if ($the_subscription->get_formatted_billing_address()) {
                                                echo '<p><strong>'.__('Address', 'wc-frontend-manager').':</strong>'.wp_kses($the_subscription->get_formatted_billing_address(), [ 'br' => [] ]).'</p>';
                                            } else {
                                    ...
                                    <td>
                                        ...
                                        if (apply_filters('wcfm_allow_customer_shipping_details', true)) {
                                            if ($the_subscription->get_formatted_shipping_address()) {
                                                echo '<p><strong>'.__('Address', 'wc-frontend-manager').':</strong>'.wp_kses($the_subscription->get_formatted_shipping_address(), [ 'br' => [] ]).'</p>';
                                            } else {
    ...
    <?php
                                    echo wp_kses(
                                        Hforce_Date_Time_Utils::hf_date_input($the_subscription->get_time($internal_date_key, 'site'), [ 'name_attr' => $date_key ]),
                                        [
                                            'input' => [
                                                'type'        => [],
                                                'class'       => [],
                  ...
                  <tbody id="order_line_items">
                      ...
                      $line_items = apply_filters('wcfm_valid_line_items', $line_items, $order->get_id());
                        foreach ($line_items as $item_id => $item) {
                            ...
                            $thumbnail = $_product ? apply_filters('woocommerce_admin_order_item_thumbnail', $_product->get_image('thumbnail', [ 'title' => '' ], false), $item_id, $item) : '';
                            ...
                            <tr class="item <?php echo apply_filters('woocommerce_admin_html_order_item_class', ( ! empty($class) ? $class : '' ), $item, $order); ?>" data-order_item_id="<?php echo $item_id; ?>">
                                ...
                                <td class="item_cost no_mob" width="1%" data-sort-value="<?php echo esc_attr($order->get_item_subtotal($item, false, true)); ?>">
                                    <div class="view">
                                        <?php
                                        if ($item->get_total()) {
                                            echo wc_price($order->get_item_total($item, false, true), [ 'currency' => $order->get_currency() ]);

                                            if ($item->get_subtotal() != $item->get_total()) {
                                                echo '<span class="wc-order-item-discount">-'.wc_price(wc_format_decimal(($order->get_item_subtotal($item, false, false) - $order->get_item_total($item, false, false)), ''), [ 'currency' => $order->get_currency() ]).'</span>';
                                            }
                                        }
                                        ?>
                                    </div>
                                ...
                                <?php if ($is_wcfm_order_details_line_total = apply_filters('wcfm_order_details_line_total', true)) { ?>
                                    <td class="line_cost" width="1%" data-sort-value="<?php echo esc_attr(( $item->get_total() ) ? $item->get_total() : ''); ?>">
                                        <div class="view">
                                            ...
                                            if ($item->get_subtotal() !== $item->get_total()) {
                                                echo '<span class="wc-order-item-discount">-'.wc_price(wc_format_decimal(($item->get_subtotal() - $item->get_total()), ''), [ 'currency' => $order->get_currency() ]).'</span>';
                                            }
                                            ...
                                            foreach ($order_taxes as $tax_item) {
                                                ...
                                                ?>
                                                    <td class="line_tax no_ipad no_mob" width="1%">
                                                        ...
                                                        <?php
                                                        if ('' != $tax_item_total) {
                                                            echo wc_price(wc_round_tax_total($tax_item_total), [ 'currency' => $order->get_currency() ]);
                                                        } else {
                                                            echo '&ndash;';
                                                        }

                                                        if ($item->get_subtotal() !== $item->get_total()) {
                                                            echo '<span class="wc-order-item-discount">-'.wc_price(wc_round_tax_total($tax_item_subtotal - $tax_item_total), [ 'currency' => $order->get_currency() ]).'</span>';
                                                        }
                        ...
                        foreach ($line_items_shipping as $item_id => $item) {
                            ...
                            <tr class="shipping <?php echo ( ! empty($class) ) ? $class : ''; ?>" data-order_item_id="<?php echo $item_id; ?>">
                                ...
                                <td class="line_cost" width="1%">
                                    <div class="view">
                                        <?php
                                        echo ( isset($item['cost']) ) ? wc_price(wc_round_tax_total($item['cost']), [ 'currency' => $order->get_currency() ]) : '';
                        ...
                        foreach ($line_items_fee as $item_id => $item) {
                            ...
                            <tr class="fee <?php echo ( ! empty($class) ) ? $class : ''; ?>" data-order_item_id="<?php echo $item_id; ?>">
                                ...
                                <td class="line_cost" width="1%">
                                    <div class="view">
                                        <?php
                                        echo ( $item->get_total() ) ? wc_price(wc_round_tax_total($item->get_total()), [ 'currency' => $order->get_currency() ]) : '';
    ...
    <?php
                        $coupons = $the_subscription->get_items([ 'coupon' ]);
                        if ($coupons) {
                            ?>
                                <div class="wc-used-coupons">
                                    ...
                                    foreach ($coupons as $item_id => $item) {
                                        ...
                                        echo '<li class="code"><a href="'.esc_url($link).'" class="img_tip" data-tip="'.esc_attr(wc_price($item['discount_amount'], [ 'currency' => $order->get_currency() ])).'"><span>'.esc_html($item->get_name()).'</span></a></li>';
                                    }
                                    ?>
                                    </ul>
                                </div>
                    ...
                    <table class="wc-order-totals">
                        <?php if ($is_wcfm_order_details_coupon_line_item = apply_filters('wcfm_order_details_coupon_line_item', true)) { ?>
                            <tr>
                                ...
                                <td width="1%"></td>
                                <td class="total">
                                    <?php echo wc_price($the_subscription->get_total_discount(), [ 'currency' => $the_subscription->get_currency() ]); ?>
                                </td>
                            </tr>
                        <?php } ?>
                        ...
                        <?php if ($is_wcfm_order_details_shipping_line_item = apply_filters('wcfm_order_details_shipping_line_item', true)) { ?>
                            <tr>
                                ...
                                if (( $refunded = $the_subscription->get_total_shipping_refunded() ) > 0) {
                                    echo '<del>'.strip_tags(wc_price($the_subscription->get_total_shipping(), [ 'currency' => $the_subscription->get_currency() ])).'</del> <ins>'.wc_price(($the_subscription->get_total_shipping() - $refunded), [ 'currency' => $the_subscription->get_currency() ]).'</ins>';
                                } else {
                                    echo wc_price($the_subscription->get_total_shipping(), [ 'currency' => $the_subscription->get_currency() ]);
                                }
                                ?>
                                </td>
                            </tr>
                                ...
                                <?php foreach ($the_subscription->get_tax_totals() as $code => $tax) : ?>
                                    <tr>
                                        ...
                                        if (( $refunded = $the_subscription->get_total_tax_refunded_by_rate_id($tax->rate_id) ) > 0) {
                                            echo '<del>'.strip_tags($tax->formatted_amount).'</del> <ins>'.wc_price((WC_Tax::round($tax->amount, wc_get_price_decimals()) - WC_Tax::round($refunded, wc_get_price_decimals())), [ 'currency' => $order->get_currency() ]).'</ins>';
                                        } else {
                        ...
                        <?php if ($is_wcfm_order_details_refund_line_item = apply_filters('wcfm_order_details_refund_line_item', true)) { ?>
                            <?php if ($the_subscription->get_total_refunded()) : ?>
                                <tr>
                                    ...
                                    <td width="1%"></td>
                                    <td class="total refunded-total">-<?php echo wc_price($the_subscription->get_total_refunded(), [ 'currency' => $the_subscription->get_currency() ]); ?></td>
                                </tr>
                            <?php endif; ?>
                        <?php } ?>
