...
if (isset($wp->query_vars['wcfm-products-manage']) && ! empty($wp->query_vars['wcfm-products-manage'])) {
    ...
    if ($product_id) {
        ...
        if ($_yith_auction_for) {
            $_yith_auction_for = get_date_from_gmt(date('Y-m-d H:i:s', absint($_yith_auction_for)));
        }

        if ($_yith_auction_to) {
            $_yith_auction_to = get_date_from_gmt(date('Y-m-d H:i:s', absint($_yith_auction_to)));
        }
    }//end if
}//end if
    ...
    <div id="wcfm_products_manage_form_yithauction_expander" class="wcfm-content">
        ...
        $WCFM->wcfm_fields->wcfm_generate_form_field(
            apply_filters(
                ...
                [
                    '_yith_auction_start_price'                     => [
                        'label'       => __('Starting Price', 'wc-frontend-manager-ultimate').'('.get_woocommerce_currency_symbol().')',
                        'type'        => 'text',
                        'class'       => 'wcfm-text wcfm_ele auction',
                        'label_class' => 'wcfm_title auction',
                        'value'       => $_yith_auction_start_price,
                    ],
                    '_yith_auction_bid_increment'                   => [
                        'label'       => __('Bid up', 'wc-frontend-manager-ultimate').'('.get_woocommerce_currency_symbol().')',
                        'type'        => 'text',
                        'class'       => 'wcfm-text wcfm_ele auction',
                        'label_class' => 'wcfm_title auction',
                        'value'       => $_yith_auction_bid_increment,
                    ],
                    '_yith_auction_minimum_increment_amount'        => [
                        'label'       => __('Minimum increment amount', 'wc-frontend-manager-ultimate').'('.get_woocommerce_currency_symbol().')',
                        'type'        => 'text',
                        'class'       => 'wcfm-text wcfm_ele auction',
                        'label_class' => 'wcfm_title auction',
                        'value'       => $_yith_auction_minimum_increment_amount,
                    ],
                    '_yith_auction_reserve_price'                   => [
                        'label'       => __('Reserve price', 'wc-frontend-manager-ultimate').'('.get_woocommerce_currency_symbol().')',
                        'type'        => 'text',
                        'class'       => 'wcfm-text wcfm_ele auction',
                        'label_class' => 'wcfm_title auction',
                        'value'       => $_yith_auction_reserve_price,
                    ],
                    '_yith_auction_buy_now'                         => [
                        'label'       => __('Buy it now price', 'wc-frontend-manager-ultimate').'('.get_woocommerce_currency_symbol().')',
                        'type'        => 'text',
                        'class'       => 'wcfm-text wcfm_ele auction',
                        'label_class' => 'wcfm_title auction',
                        'value'       => $_yith_auction_buy_now,
                    ],
                    '_yith_auction_for'                             => [
                        'label'       => __('Auction Date From', 'wc-frontend-manager-ultimate'),
                        'type'        => 'text',
                        'placeholder' => 'YYYY-MM-DD hh:mm:ss',
                        'class'       => 'wcfm-text wcfm_ele auction',
                        'label_class' => 'wcfm_title auction',
                        'value'       => $_yith_auction_for,
                    ],
                    '_yith_auction_to'                              => [
                        'label'       => __('Auction Date To', 'wc-frontend-manager-ultimate'),
                        'type'        => 'text',
                        'placeholder' => 'YYYY-MM-DD hh:mm:ss',
                        'class'       => 'wcfm-text wcfm_ele auction',
                        'label_class' => 'wcfm_title auction',
                        'value'       => $_yith_auction_to,
                    ],
                    '_yith_check_time_for_overtime_option'          => [
                        'label'       => __('Time to add overtime', 'wc-frontend-manager-ultimate'),
                        'type'        => 'number',
                        'class'       => 'wcfm-text wcfm_ele auction',
                        'label_class' => 'wcfm_title auction',
                        'value'       => $_yith_check_time_for_overtime_option,
                        'hints'       => __('Number of minutes before auction ends to check if overtime added. (Override the settings option)', 'wc-frontend-manager-ultimate'),
                    ],
                    '_yith_overtime_option'                         => [
                        'label'       => __('Overtime', 'wc-frontend-manager-ultimate'),
                        'type'        => 'text',
                        'class'       => 'wcfm-text wcfm_ele auction',
                        'label_class' => 'wcfm_title auction',
                        'value'       => $_yith_overtime_option,
                        'hints'       => __('Number of minutes by which the auction will be extended. (Overrride the settings option)', 'wc-frontend-manager-ultimate'),
                    ],
                    '_yith_wcact_auction_automatic_reschedule'      => [
                        'label'       => __('Time value for automatic rescheduling', 'wc-frontend-manager-ultimate'),
                        'type'        => 'text',
                        'class'       => 'wcfm-text wcfm_ele auction',
                        'label_class' => 'wcfm_title auction',
                        'value'       => $_yith_wcact_auction_automatic_reschedule,
                    ],
                    '_yith_wcact_automatic_reschedule_auction_unit' => [
                        'label'       => __('Select unit for automatic rescheduling', 'wc-frontend-manager-ultimate'),
                        'type'        => 'select',
                        'class'       => 'wcfm-select wcfm_ele auction',
                        'label_class' => 'wcfm_title auction',
                        'options'     => [
                            'days'    => __('days', 'wc-frontend-manager-ultimate'),
                            'hours'   => __('hours', 'wc-frontend-manager-ultimate'),
                            'minutes' => __('minutes', 'wc-frontend-manager-ultimate'),
                        ],
                        'value'       => $_yith_wcact_automatic_reschedule_auction_unit,
                    ],
                    '_yith_wcact_upbid_checkbox'                    => [
                        'label'       => __('Show bid up', 'wc-frontend-manager-ultimate'),
                        'type'        => 'checkbox',
                        'class'       => 'wcfm-checkbox wcfm_ele auction',
                        'label_class' => 'wcfm_title auction',
                        'value'       => 'yes',
                        'dfvalue'     => $_yith_wcact_upbid_checkbox,
                    ],
                    '_yith_wcact_overtime_checkbox'                 => [
                        'label'       => __('Show overtime', 'wc-frontend-manager-ultimate'),
                        'type'        => 'checkbox',
                        'class'       => 'wcfm-checkbox wcfm_ele auction',
                        'label_class' => 'wcfm_title auction',
                        'value'       => 'yes',
                        'dfvalue'     => $_yith_wcact_overtime_checkbox,
                    ],
                ],
                $product_id
            )
