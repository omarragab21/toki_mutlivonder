...
$_woo_vou_recipient_delivery_offline_recipient = [];
...
if (isset($wp->query_vars['wcfm-products-manage']) && ! empty($wp->query_vars['wcfm-products-manage'])) {
    ...
    if ($product_id) {
        ...
        $_woo_vou_recipient_delivery_email_recipient   = isset($_woo_vou_recipient_delivery['email']) ? $_woo_vou_recipient_delivery['email'] : [];
        ...
        $_woo_vou_recipient_delivery_offline_recipient = isset($_woo_vou_recipient_delivery['offline']) ? $_woo_vou_recipient_delivery['offline'] : [];
        ...
        if (! $_woo_vou_pdf_template_selection) {
            $_woo_vou_pdf_template_selection = [];
        }
        ...
        if (! $_woo_vou_disable_redeem_day) {
            $_woo_vou_disable_redeem_day = [];
        }

        $_woo_vou_logo = get_post_meta($product_id, '_woo_vou_logo', true);
        if ($_woo_vou_logo && isset($_woo_vou_logo['src'])) {
            $_woo_vou_logo = $_woo_vou_logo['src'];
        }
...
$voucher_preview_opt  = $multiple_pdf_opt = $coupon_code_opt = [
    ''    => __('Default', 'woovoucher'),
...
$expdate_types = apply_filters(
    'woo_vou_exp_date_types',
    [
        'default'           => __('Default', 'woovoucher'),
        'specific_date'     => __('Specific Time', 'woovoucher'),
        'based_on_purchase' => __(
            'Based on Purchase',
            'woovoucher'
        ),
    ]
...
$voucher_options          = [ '' => __('Please Select', 'woovoucher') ];
...
if (wcfm_is_vendor()) {
    ...
    $vendor_options        = [ $_woo_vou_vendor_user => 'ME' ];
...
<div class="wcfm-container simple variable downlodable">
    <div id="wcfm_products_manage_form_wc_pdf_vouchers_expander" class="wcfm-content">
        <?php
        $woo_vou_fields = apply_filters(
            'wcfm_product_manage_fields_wc_pdf_vouchers',
            [
                '_woo_vou_heading_1'                            => [
                    'type'  => 'html',
                    'value' => '<h2>'.__('Voucher General Setting', 'wc-frontend-manager-ultimate').'</h2><div class="wcfm_clearfix"></div>',
                ],
                ...
                '_woo_vou_enable'                               => [
                    ...
                    'hints'       => __('To enable the voucher for this product check the "Enable Voucher Codes" check box.', 'woovoucher'),
                    'dfvalue'     => $_woo_vou_enable,
                    'value'       => 'yes',
                ],

                '_woo_vou_voucher_price'                        => [
                    'label'       => __('Voucher price', 'woovoucher').'('.get_woocommerce_currency_symbol().')',
                    'type'        => 'number',
                    'class'       => 'wcfm-text wcfm_non_negative_input',
                    'label_class' => 'wcfm_title',
                    'value'       => $_woo_vou_voucher_price,
                ],

                '_woo_vou_product_start_date'                   => [
                    'label'             => __('Product Start Date:', 'woovoucher'),
                    'type'              => 'text',
                    'class'             => 'wcfm-text',
                    'label_class'       => 'wcfm_title',
                    'hints'             => __('If you want to make the product valid for a specific time only, you can enter an start date here.', 'woovoucher'),
                    'custom_attributes' => [ 'date_format' => $woo_vou_pro_start_end_date_format ],
                    'value'             => $_woo_vou_product_start_date,
                ],
                '_woo_vou_product_exp_date'                     => [
                    'label'             => __('Product End Date:', 'woovoucher'),
                    'type'              => 'text',
                    'class'             => 'wcfm-text',
                    'label_class'       => 'wcfm_title',
                    'hints'             => __('If you want to make the product valid for a specific time only, you can enter an end date here.', 'woovoucher'),
                    'custom_attributes' => [ 'date_format' => $woo_vou_pro_start_end_date_format ],
                ...
                '_woo_vou_voucher_delivery'                     => [
                    ...
                    'class'       => 'wcfm-select ',
                    'label_class' => 'wcfm_title ',
                    'desc_class'  => 'wc_pdf_vouchers_desc',
                    'desc'        => sprintf(__('Choose how your customer receives the "PDF Voucher" %1$sEmail%2$s - Customer receives "PDF Voucher" through email. %3$sOffline%4$s - You will have to send voucher through physical mode, via post or on-shop. %5$sThis setting modifies the global voucher delivery setting and overrides voucher\'s delivery value. Set delivery "%6$sDefault%7$s" to use the global/voucher settings.', 'woovoucher'), '<br /><b>', '</b>', '<br /><b>', '</b>', '<br />', '<b>', '</b>'),
                    'options'     => $voucher_delivery_opt,
                    'value'       => $_woo_vou_voucher_delivery,
                ],
                '_break_ele3'                                   => [
                    'type'  => 'html',
                    'value' => '<div class="wcfm-clearfix"></div>',
                ...
                '_woo_vou_enable_pdf_preview'                   => [
                    'label'       => __('Enable Voucher Preview:', 'woovoucher'),
                    'type'        => 'select',
                    'class'       => 'wcfm-select ',
                    'label_class' => 'wcfm_title ',
                    'desc_class'  => 'wc_pdf_vouchers_desc',
                    'desc'        => __('Choose Yes / No to allow / disallow users to preview the voucher on product detail page before placing the order. Leave it empty to use global settings.', 'woovoucher'),
                    'options'     => $voucher_preview_opt,
                    'value'       => $_woo_vou_enable_pdf_preview,
                ],
                '_break_ele4'                                   => [
                    'type'  => 'html',
                    'value' => '<div class="wcfm-clearfix"></div>',
                ...
                '_woo_vou_enable_multiple_pdf'                  => [
                    'label'       => __('Enable 1 voucher per PDF:', 'woovoucher'),
                    'type'        => 'select',
                    'class'       => 'wcfm-select ',
                    'label_class' => 'wcfm_title ',
                    'desc_class'  => 'wc_pdf_vouchers_desc',
                    'desc'        => __('Choose Yes if you want to generate 1 PDF for 1 voucher code, choose No if you want to generate 1 combined PDF for all vouchers, choose Default to use global settings.', 'woovoucher'),
                ...
                '_woo_vou_using_type'                           => [
                    'label'       => __('Usability:', 'woovoucher'),
                    'type'        => 'select',
                    'class'       => 'wcfm-select ',
                    'label_class' => 'wcfm_title ',
                    'desc_class'  => 'wc_pdf_vouchers_desc',
                    'desc'        => sprintf(__('Choose how you wanted to use vouchers codes. %1$sIf you set usability "%2$sOne time only%3$s" then it will automatically set product quantity equal to a number of voucher codes entered and it will automatically decrease quantity  by 1 when it gets purchased. If you set usability "%4$sUnlimited%5$s" then the plugin will automatically generate unique voucher codes when the product purchased. %6$sThis setting modifies the global usability setting and overrides vendor\'s usability value. Set usability "%7$sDefault%8$s" to use the global/vendor settings.', 'woovoucher'), '<br />', '<b>', '</b>', '<b>', '</b>', '<br />', '<b>', '</b>'),
                    'options'     => $using_type_opt,
                    'value'       => $_woo_vou_using_type,
                ],
                '_break_ele7'                                   => [
                    'type'  => 'html',
                    'value' => '<div class="wcfm-clearfix"></div>',
                ...
                '_woo_vou_codes'                                => [
                    ...
                    'class'       => 'wcfm-textarea ',
                    'label_class' => 'wcfm_title ',
                    'hints'       => __('If you have a list of voucher codes you can copy and paste them into this option. Make sure, that they are comma separated.', 'woovoucher'),
                    'value'       => $_woo_vou_codes,
                ],
                '_woo_vou_generate_codes'                       => [
                    'label'       => __('Generate Codes', 'woovoucher'),
                    'label_class' => 'wcfm_title ',
                    'type'        => 'html',
                    'value'       => '<input type="button" class="button wcfm_submit_button wcfm_voucher_code_popup" value="'.__('Generate Codes', 'woovoucher').'" />',
                ],
                '_break_ele8'                                   => [
                    'type'  => 'html',
                    'value' => '<div class="wcfm-clearfix"></div>',
                ...
                '_woo_vou_purchased_codes'                      => [
                    ...
                    'label_class' => 'wcfm_title woo_vou_purchased_codes_ele wcfm_ele_hide',
                    'type'        => 'html',
                    'value'       => '<input type="button" class="button wcfm_submit_button wcfm_voucher_purchased_code_popup woo_vou_purchased_codes_ele wcfm_ele_hide" value="'.__('Purchased Voucher Codes', 'woovoucher').'" />',
                ],
                '_break_ele8a'                                  => [
                    'type'  => 'html',
                    'value' => '<div class="wcfm-clearfix"></div>',
                ...
                '_woo_vou_used_codes'                           => [
                    ...
                    'label_class' => 'wcfm_title woo_vou_used_codes_ele wcfm_ele_hide',
                    'type'        => 'html',
                    'value'       => '<input type="button" class="button wcfm_submit_button wcfm_voucher_used_codes_popup woo_vou_used_codes_ele wcfm_ele_hide" value="'.__('Used Voucher Codes', 'woovoucher').'" />',
                ],
                '_break_ele8b'                                  => [
                    'type'  => 'html',
                    'value' => '<div class="wcfm-clearfix"></div>',
                ...
                '_woo_vou_exp_type'                             => [
                    ...
                    'label_class' => 'wcfm_title ',
                    'desc_class'  => 'wc_pdf_vouchers_desc',
                    'desc'        => sprintf(__('Please select expiration date type either a %1$sSpecific Time%2$s or set date %3$sBased on Purchased%4$s voucher date like after 7 days, 30 days, 1 year etc. %5$sThis setting modifies the global voucher expiration date setting and overrides voucher\'s expiration date value. Set expiration date type "%6$sDefault%7$s" to use the global/voucher settings.', 'woovoucher'), '<b>', '</b>', '<b>', '</b>', '<br />', '<b>', '</b>'),
                    'options'     => $expdate_types,
                    'value'       => $_woo_vou_exp_type,
                ],
                '_break_ele9'                                   => [
                    'type'  => 'html',
                    'value' => '<div class="wcfm-clearfix"></div>',
                ...
                '_woo_vou_start_date'                           => [
                    'label'             => __('Start Date:', 'woovoucher'),
                    'type'              => 'text',
                    'class'             => 'wcfm-text specific_date_ele',
                    'label_class'       => 'wcfm_title specific_date_ele',
                    'hints'             => __('If you want to make the voucher codes valid for a specific time only, you can enter a start date here.', 'woovoucher'),
                    'custom_attributes' => [ 'date_format' => $woo_vou_vou_start_end_date_format ],
                    'value'             => $_woo_vou_start_date,
                ],
                '_woo_vou_exp_date'                             => [
                    'label'             => __('Expiration Date:', 'woovoucher'),
                    'type'              => 'text',
                    'class'             => 'wcfm-text specific_date_ele',
                    'label_class'       => 'wcfm_title specific_date_ele',
                    'hints'             => __('If you want to make the voucher codes valid for a specific time only, you can enter a expiration date here. If the Voucher Code never expires, then leave that option blank.', 'woovoucher'),
                    'custom_attributes' => [ 'date_format' => $woo_vou_vou_start_end_date_format ],
                ...
                '_woo_vou_days_diff'                            => [
                    ...
                    'type'        => 'select',
                    'class'       => 'wcfm-select based_on_purchase_ele',
                    'label_class' => 'wcfm_title based_on_purchase_ele',
                    'desc'        => __(' After purchase', 'woovoucher'),
                    'desc_class'  => 'woo_vou_days_diff_desc woo_vou_days_diff_custom_non_ele',
                    'options'     => $based_on_purchase_opt,
                    'value'       => $_woo_vou_days_diff,
                ],
                '_woo_vou_custom_days'                          => [
                    'type'        => 'text',
                    'class'       => 'wcfm-text woo_vou_days_diff_custom_ele',
                    'label_class' => 'wcfm_title woo_vou_days_diff_custom_ele',
                    'desc'        => __(' Days after purchase', 'woovoucher'),
                    'desc_class'  => 'woo_vou_days_diff_desc woo_vou_days_diff_custom_ele',
                    'value'       => $_woo_vou_custom_days,
                ],

                '_woo_vou_disable_redeem_day'                   => [
                    'label'       => __('Choose Which Days Voucher can not be Used:', 'woovoucher'),
                    'type'        => 'select',
                    'class'       => 'wcfm-select',
                    'label_class' => 'wcfm_title',
                    'attributes'  => [ 'multiple' => true ],
                    'hints'       => __('If you want to restrict  use of voucher codes  for specific days, you can select days here. Leave it blank for no restriction. ', 'woovoucher'),
                    'options'     => $redeem_days,
                    'value'       => $_woo_vou_disable_redeem_day,
                ],

                '_woo_vou_heading_2'                            => [
                    'type'  => 'html',
                    'value' => '<h2 style="padding-top:50px;">'.__('Gift Voucher Setting', 'wc-frontend-manager-ultimate').'</h2><div class="wcfm_clearfix"></div>',
                ],
                ...
                '_woo_vou_enable_recipient_name'                => [
                    ...
                    'class'       => 'wcfm-checkbox',
                    'label_class' => 'wcfm_title checkbox_title',
                    'hints'       => __('To enable the recipient name on the product page.', 'woovoucher'),
                    'dfvalue'     => $_woo_vou_enable_recipient_name,
                    'value'       => 'yes',
                ],
                '_woo_vou_recipient_name_label'                 => [
                    'label'       => '&nbsp;&nbsp;',
                    'type'        => 'text',
                    'class'       => 'wcfm-text _woo_vou_enable_recipient_name_ele',
                    'label_class' => 'wcfm_title _woo_vou_enable_recipient_name_ele',
                    'placeholder' => __('Label:', 'woovoucher'),
                    'value'       => $_woo_vou_recipient_name_label,
                ],
                '_woo_vou_recipient_name_max_length'            => [
                    'type'        => 'text',
                    'class'       => 'wcfm-text _woo_vou_enable_recipient_name_ele',
                    'label_class' => 'wcfm_title checkbox_title',
                    'placeholder' => __('Max Length:', 'woovoucher'),
                    'value'       => $_woo_vou_recipient_name_max_length,
                ],
                '_woo_vou_recipient_name_is_required'           => [
                    'type'        => 'checkbox',
                    'class'       => 'wcfm-checkbox _woo_vou_enable_recipient_name_ele',
                    'label_class' => 'wcfm_title',
                    'dfvalue'     => $_woo_vou_recipient_name_is_required,
                    'value'       => 'yes',
                    'attributes'  => [ 'title' => __('Enable to make this required.', 'wc-frontend-manager-ultimate') ],
                ],
                '_woo_vou_recipient_name_desc'                  => [
                    'label'       => __('Description:', 'woovoucher'),
                    'type'        => 'textarea',
                    'class'       => 'wcfm-textarea _woo_vou_enable_recipient_name_ele',
                    'label_class' => 'wcfm_title _woo_vou_enable_recipient_name_ele',
                    'value'       => $_woo_vou_recipient_name_desc,
                    'hints'       => __('Enter the description which you want to show on product page.', 'woovoucher'),
                ],
                '_break_ele_gvs_1'                              => [
                    'type'  => 'html',
                    'value' => '<div class="wcfm-clearfix _woo_vou_enable_recipient_name_ele" style="margin-bottom:25px;"></div>',
                ],
                ...
                '_woo_vou_enable_recipient_email'               => [
                    ...
                    'class'       => 'wcfm-checkbox',
                    'label_class' => 'wcfm_title checkbox_title',
                    'hints'       => __('To enable the recipient email on the product page.', 'woovoucher'),
                    'dfvalue'     => $_woo_vou_enable_recipient_email,
                    'value'       => 'yes',
                ],
                '_woo_vou_recipient_email_label'                => [
                    'label'       => '&nbsp;&nbsp;',
                    'type'        => 'text',
                    'class'       => 'wcfm-text _woo_vou_enable_recipient_email_ele',
                    'label_class' => 'wcfm_title _woo_vou_enable_recipient_email_ele',
                    'placeholder' => __('Label:', 'woovoucher'),
                    'value'       => $_woo_vou_recipient_email_label,
                ],
                '_woo_vou_recipient_email_is_required'          => [
                    'type'        => 'checkbox',
                    'class'       => 'wcfm-checkbox _woo_vou_enable_recipient_email_ele',
                    'label_class' => 'wcfm_title',
                    'dfvalue'     => $_woo_vou_recipient_email_is_required,
                    'value'       => 'yes',
                    'attributes'  => [ 'title' => __('Enable to make this required.', 'wc-frontend-manager-ultimate') ],
                ],
                '_woo_vou_recipient_email_desc'                 => [
                    'label'       => __('Description:', 'woovoucher'),
                    'type'        => 'textarea',
                    'class'       => 'wcfm-textarea _woo_vou_enable_recipient_email_ele',
                    'label_class' => 'wcfm_title _woo_vou_enable_recipient_email_ele',
                    'value'       => $_woo_vou_recipient_email_desc,
                    'hints'       => __('Enter the description which you want to show on product page.', 'woovoucher'),
                ],
                '_break_ele_gvs_2'                              => [
                    'type'  => 'html',
                    'value' => '<div class="wcfm-clearfix _woo_vou_enable_recipient_email_ele" style="margin-bottom:25px;"></div>',
                ],
                ...
                '_woo_vou_enable_recipient_message'             => [
                    ...
                    'class'       => 'wcfm-checkbox',
                    'label_class' => 'wcfm_title checkbox_title',
                    'hints'       => __('To enable the recipient message on the product page.', 'woovoucher'),
                    'dfvalue'     => $_woo_vou_enable_recipient_message,
                    'value'       => 'yes',
                ],
                '_woo_vou_recipient_message_label'              => [
                    'label'       => '&nbsp;&nbsp;',
                    'type'        => 'text',
                    'class'       => 'wcfm-text _woo_vou_enable_recipient_message_ele',
                    'label_class' => 'wcfm_title _woo_vou_enable_recipient_message_ele',
                    'placeholder' => __('Label:', 'woovoucher'),
                    'value'       => $_woo_vou_recipient_message_label,
                ],
                '_woo_vou_recipient_message_max_length'         => [
                    'type'        => 'text',
                    'class'       => 'wcfm-text _woo_vou_enable_recipient_message_ele',
                    'label_class' => 'wcfm_title checkbox_title',
                    'placeholder' => __('Max Length:', 'woovoucher'),
                    'value'       => $_woo_vou_recipient_message_max_length,
                ],
                '_woo_vou_recipient_message_is_required'        => [
                    'type'        => 'checkbox',
                    'class'       => 'wcfm-checkbox _woo_vou_enable_recipient_message_ele',
                    'label_class' => 'wcfm_title',
                    'dfvalue'     => $_woo_vou_recipient_message_is_required,
                    'value'       => 'yes',
                    'attributes'  => [ 'title' => __('Enable to make this required.', 'wc-frontend-manager-ultimate') ],
                ],
                '_woo_vou_recipient_message_desc'               => [
                    'label'       => __('Description:', 'woovoucher'),
                    'type'        => 'textarea',
                    'class'       => 'wcfm-textarea _woo_vou_enable_recipient_message_ele',
                    'label_class' => 'wcfm_title _woo_vou_enable_recipient_message_ele',
                    'value'       => $_woo_vou_recipient_message_desc,
                    'hints'       => __('Enter the description which you want to show on product page.', 'woovoucher'),
                ],
                '_break_ele_gvs_3'                              => [
                    'type'  => 'html',
                    'value' => '<div class="wcfm-clearfix _woo_vou_enable_recipient_message_ele" style="margin-bottom:25px;"></div>',
                ],

                '_woo_vou_enable_recipient_giftdate'            => [
                    'label'       => __('Enable Recipient Gift Date:', 'woovoucher'),
                    'type'        => 'checkbox',
                    'class'       => 'wcfm-checkbox',
                    'label_class' => 'wcfm_title checkbox_title',
                    'hints'       => __('To enable the recipient\'s gift date selection on the product page.', 'woovoucher'),
                    'dfvalue'     => $_woo_vou_enable_recipient_giftdate,
                    'value'       => 'yes',
                ],
                '_woo_vou_recipient_giftdate_label'             => [
                    'label'       => '&nbsp;&nbsp;',
                    'type'        => 'text',
                    'class'       => 'wcfm-text _woo_vou_enable_recipient_giftdate_ele',
                    'label_class' => 'wcfm_title _woo_vou_enable_recipient_giftdate_ele',
                    'placeholder' => __('Label:', 'woovoucher'),
                    'value'       => $_woo_vou_recipient_giftdate_label,
                ],
                '_woo_vou_recipient_giftdate_is_required'       => [
                    'type'        => 'checkbox',
                    'class'       => 'wcfm-checkbox _woo_vou_enable_recipient_giftdate_ele',
                    'label_class' => 'wcfm_title',
                    'dfvalue'     => $_woo_vou_recipient_giftdate_is_required,
                    'value'       => 'yes',
                    'attributes'  => [ 'title' => __('Enable to make this required.', 'wc-frontend-manager-ultimate') ],
                ],
                '_woo_vou_recipient_giftdate_desc'              => [
                    'label'       => __('Description:', 'woovoucher'),
                    'type'        => 'textarea',
                    'class'       => 'wcfm-textarea _woo_vou_enable_recipient_giftdate_ele',
                    'label_class' => 'wcfm_title _woo_vou_enable_recipient_giftdate_ele',
                    'value'       => $_woo_vou_recipient_giftdate_desc,
                    'hints'       => __('Enter the description which you want to show on product page.', 'woovoucher'),
                ],
                '_break_ele_gvs_4'                              => [
                    'type'  => 'html',
                    'value' => '<div class="wcfm-clearfix _woo_vou_enable_recipient_giftdate_ele" style="margin-bottom:25px;"></div>',
                ],

                '_woo_vou_enable_recipient_delivery_method'     => [
                    'label'       => __('Enable Delivery Method:', 'woovoucher'),
                    'type'        => 'checkbox',
                    'class'       => 'wcfm-checkbox',
                    'label_class' => 'wcfm_title checkbox_title',
                    'hints'       => __('To enable the recipient\'s delivery method on the product page.', 'woovoucher'),
                    'dfvalue'     => $_woo_vou_enable_recipient_delivery_method,
                    'value'       => 'yes',
                ],
                '_woo_vou_recipient_delivery_label'             => [
                    'label'       => '&nbsp;&nbsp;',
                    'type'        => 'text',
                    'class'       => 'wcfm-text _woo_vou_enable_recipient_delivery_ele',
                    'label_class' => 'wcfm_title _woo_vou_enable_recipient_delivery_ele',
                    'placeholder' => __('Label:', 'woovoucher'),
                    'value'       => $_woo_vou_recipient_delivery_label,
                ],
                '_break_ele_gvs_5'                              => [
                    'type'  => 'html',
                    'value' => '<div class="wcfm-clearfix _woo_vou_enable_recipient_delivery_ele" style="margin-bottom:25px;"></div>',
                ],

                '_woo_vou_recipient_delivery_email_enable'      => [
                    'label'       => __('Email to Recipient', 'woovoucher'),
                    'type'        => 'checkbox',
                    'name'        => '_woo_vou_recipient_delivery[enable_email]',
                    'class'       => 'wcfm-checkbox _woo_vou_enable_recipient_delivery_ele',
                    'label_class' => 'wcfm_title _woo_vou_enable_recipient_delivery_ele',
                    'dfvalue'     => $_woo_vou_recipient_delivery_email_enable,
                    'value'       => 'yes',
                ],
                '_woo_vou_recipient_delivery_email_label'       => [
                    'label'       => '&nbsp;&nbsp;',
                    'name'        => '_woo_vou_recipient_delivery[label_email]',
                    'type'        => 'text',
                    'class'       => 'wcfm-text _woo_vou_enable_recipient_delivery_ele _woo_vou_enable_recipient_delivery_email_ele',
                    'label_class' => 'wcfm_title _woo_vou_enable_recipient_delivery_ele _woo_vou_enable_recipient_delivery_email_ele',
                    'placeholder' => __('Label:', 'woovoucher'),
                    'value'       => $_woo_vou_recipient_delivery_email_label,
                ],
                '_break_ele_gvs_6'                              => [
                    'type'  => 'html',
                    'value' => '<div class="wcfm-clearfix _woo_vou_enable_recipient_delivery_ele _woo_vou_enable_recipient_delivery_email_ele"></div>',
                ],
                '_woo_vou_recipient_delivery_email_desc'        => [
                    'label'       => __('Description:', 'woovoucher'),
                    'name'        => '_woo_vou_recipient_delivery[desc_email]',
                    'type'        => 'textarea',
                    'class'       => 'wcfm-textarea _woo_vou_enable_recipient_delivery_ele _woo_vou_enable_recipient_delivery_email_ele',
                    'label_class' => 'wcfm_title _woo_vou_enable_recipient_delivery_ele _woo_vou_enable_recipient_delivery_email_ele',
                    'value'       => $_woo_vou_recipient_delivery_email_desc,
                    'hints'       => __('Enter the description which you want to show on product page.', 'woovoucher'),
                ],
                '_woo_vou_recipient_delivery_email_recipient'   => [
                    'label'       => __('Recipient Fields:', 'wc-frontend-manager-ultimate'),
                    'name'        => '_woo_vou_recipient_delivery[email]',
                    'type'        => 'select',
                    'options'     => $_recipient_details,
                    'attributes'  => [ 'multiple' => true ],
                    'class'       => 'wcfm-select _woo_vou_enable_recipient_delivery_ele _woo_vou_enable_recipient_delivery_email_ele',
                    'label_class' => 'wcfm_title _woo_vou_enable_recipient_delivery_ele _woo_vou_enable_recipient_delivery_email_ele',
                    'value'       => $_woo_vou_recipient_delivery_email_recipient,
                    'hints'       => __('Select recipient fields that you want to show in Email to Recipient block.', 'wc-frontend-manager-ultimate'),
                ],
                '_break_ele_gvs_7'                              => [
                    'type'  => 'html',
                    'value' => '<div class="wcfm-clearfix _woo_vou_enable_recipient_delivery_ele _woo_vou_enable_recipient_delivery_email_ele" style="margin-bottom:25px;"></div>',
                ],

                '_woo_vou_recipient_delivery_offline_enable'    => [
                    'label'       => __('Offline', 'woovoucher'),
                    'type'        => 'checkbox',
                    'name'        => '_woo_vou_recipient_delivery[enable_offline]',
                    'class'       => 'wcfm-checkbox _woo_vou_enable_recipient_delivery_ele',
                    'label_class' => 'wcfm_title _woo_vou_enable_recipient_delivery_ele',
                    'dfvalue'     => $_woo_vou_recipient_delivery_offline_enable,
                    'value'       => 'yes',
                ],
                '_woo_vou_recipient_delivery_offline_label'     => [
                    'label'       => '&nbsp;&nbsp;',
                    'name'        => '_woo_vou_recipient_delivery[label_offline]',
                    'type'        => 'text',
                    'class'       => 'wcfm-text _woo_vou_enable_recipient_delivery_ele _woo_vou_enable_recipient_delivery_offline_ele',
                    'label_class' => 'wcfm_title _woo_vou_enable_recipient_delivery_ele _woo_vou_enable_recipient_delivery_offline_ele',
                    'placeholder' => __('Label:', 'woovoucher'),
                    'value'       => $_woo_vou_recipient_delivery_offline_label,
                ],
                '_break_ele_gvs_8'                              => [
                    'type'  => 'html',
                    'value' => '<div class="wcfm-clearfix _woo_vou_enable_recipient_delivery_ele _woo_vou_enable_recipient_delivery_offline_ele"></div>',
                ],
                '_woo_vou_recipient_delivery_offline_desc'      => [
                    'label'       => __('Description:', 'woovoucher'),
                    'name'        => '_woo_vou_recipient_delivery[desc_offline]',
                    'type'        => 'textarea',
                    'class'       => 'wcfm-textarea _woo_vou_enable_recipient_delivery_ele _woo_vou_enable_recipient_delivery_offline_ele',
                    'label_class' => 'wcfm_title _woo_vou_enable_recipient_delivery_ele _woo_vou_enable_recipient_delivery_offline_ele',
                    'value'       => $_woo_vou_recipient_delivery_offline_desc,
                    'hints'       => __('Enter the description which you want to show on product page.', 'woovoucher'),
                ],
                '_woo_vou_recipient_delivery_offline_recipient' => [
                    'label'       => __('Recipient Fields:', 'wc-frontend-manager-ultimate'),
                    'name'        => '_woo_vou_recipient_delivery[offline]',
                    'type'        => 'select',
                    'options'     => $_recipient_details,
                    'attributes'  => [ 'multiple' => true ],
                    'class'       => 'wcfm-select _woo_vou_enable_recipient_delivery_ele _woo_vou_enable_recipient_delivery_offline_ele',
                    'label_class' => 'wcfm_title _woo_vou_enable_recipient_delivery_ele _woo_vou_enable_recipient_delivery_offline_ele',
                    'value'       => $_woo_vou_recipient_delivery_offline_recipient,
                    'hints'       => __('Select recipient fields that you want to show in Offline to Recipient block.', 'wc-frontend-manager-ultimate'),
                ],


                '_woo_vou_heading_3'                            => [
                    'type'  => 'html',
                    'value' => '<h2 style="padding-top:50px;">'.__('Voucher Template Setting', 'wc-frontend-manager-ultimate').'</h2><div class="wcfm_clearfix"></div>',
                ],

                '_woo_vou_enable_pdf_template_selection'        => [
                    'label'       => __('Enable Template Selection:', 'woovoucher'),
                    'type'        => 'checkbox',
                    'class'       => 'wcfm-checkbox',
                    'label_class' => 'wcfm_title checkbox_title',
                    'hints'       => __('To enable the PDF template selection on the product page.', 'woovoucher'),
                    'dfvalue'     => $_woo_vou_enable_pdf_template_selection,
                    'value'       => 'yes',
                ],
                '_woo_vou_pdf_template_selection_label'         => [
                    'label'       => '&nbsp;&nbsp;',
                    'type'        => 'text',
                    'class'       => 'wcfm-text _woo_vou_enable_pdf_template_selection_ele',
                    'label_class' => 'wcfm_title _woo_vou_enable_pdf_template_selection_ele',
                    'placeholder' => __('Label:', 'woovoucher'),
                    'value'       => $_woo_vou_pdf_template_selection_label,
                ],
                '_woo_vou_pdf_template_selection'               => [
                    'label'       => __('Select PDF Template:', 'woovoucher'),
                    'type'        => 'select',
                    'class'       => 'wcfm-select _woo_vou_enable_pdf_template_selection_ele',
                    'label_class' => 'wcfm_title _woo_vou_enable_pdf_template_selection_ele',
                    'attributes'  => [ 'multiple' => true ],
                    'options'     => $multiple_voucher_options,
                    'value'       => $_woo_vou_pdf_template_selection,
                ],
                '_woo_vou_pdf_selection_desc'                   => [
                    'label'       => __('Description:', 'woovoucher'),
                    'type'        => 'textarea',
                    'class'       => 'wcfm-textarea _woo_vou_enable_pdf_template_selection_ele',
                    'label_class' => 'wcfm_title _woo_vou_enable_pdf_template_selection_ele',
                    'value'       => $_woo_vou_pdf_selection_desc,
                    'hints'       => __('Enter the description which you want to show on product page.', 'woovoucher'),
                ],
                '_break_ele_gvs_70'                             => [
                    'type'  => 'html',
                    'value' => '<div class="wcfm-clearfix _woo_vou_enable_pdf_template_selection_non_ele"></div>',
                ],
                '_woo_vou_pdf_template'                         => [
                    'label'       => __('PDF Template:', 'woovoucher'),
                    'type'        => 'select',
                    'class'       => 'wcfm-select _woo_vou_enable_pdf_template_selection_non_ele',
                    'label_class' => 'wcfm_title _woo_vou_enable_pdf_template_selection_non_ele',
                    'hints'       => __('Select a PDF template. This setting modifies the global PDF template setting and overrides vendor\'s PDF template value. Leave it empty to use the global/vendor settings.', 'woovoucher'),
                ...
                '_woo_vou_heading_4'                            => [
                    'type'  => 'html',
                    'value' => '<h2 style="padding-top:50px;">'.__('Voucher Vendor Setting', 'wc-frontend-manager-ultimate').'</h2><div class="wcfm_clearfix"></div>',
                ],

                '_woo_vou_vendor_user'                          => [
                    'label'       => __('Primary Vendor User:', 'woovoucher'),
                    'type'        => 'select',
                    'class'       => 'wcfm-select '.$vendor_user_ele_class,
                    'label_class' => 'wcfm_title '.$vendor_user_ele_class,
                    'hints'       => __('Please select the primary vendor user.', 'woovoucher'),
                    'options'     => $vendor_options,
                    'value'       => $_woo_vou_vendor_user,
                ],
                '_woo_vou_sec_vendor_users'                     => [
                    'label'       => __('Secondary Vendor Users:', 'woovoucher'),
                    'type'        => 'select',
                    'class'       => 'wcfm-select '.$vendor_user_ele_class,
                    'label_class' => 'wcfm_title '.$vendor_user_ele_class,
                    'attributes'  => [ 'multiple' => true ],
                    'hints'       => __('Please select the secondary vendor users. You can select multiple users as secondary vendor users.', 'woovoucher'),
                    'options'     => $vendor_options,
                    'value'       => $_woo_vou_sec_vendor_users,
                ],

                '_woo_vou_logo'                                 => [
                    'label'                => __('Vendor\'s Logo:', 'woovoucher'),
                    'type'                 => 'upload',
                    'class'                => 'wcfm-text',
                    'label_class'          => 'wcfm_title',
                    'prwidth'              => 150,
                    'wcfm_uploader_by_url' => true,
                    'hints'                => __('Allows you to upload a logo of the vendor for which this voucher is valid. The logo will also be displayed on the PDF document. Leave it empty to use the vendor logo from the vendor settings.', 'woovoucher'),
                    'value'                => $_woo_vou_logo,
                ],
                '_woo_vou_address_phone'                        => [
                    ...
                    'class'       => 'wcfm-textarea ',
                    'label_class' => 'wcfm_title ',
                    'hints'       => __('Here you can enter the complete vendor\'s address. This will be displayed on the PDF document sent to the customers so that they know where to redeem this voucher. Limited HTML is allowed. Leave it empty to use address from the vendor settings.', 'woovoucher'),
                    'value'       => $_woo_vou_address_phone,
                ],
                '_woo_vou_website'                              => [
                    ...
                    'class'       => 'wcfm-text',
                    'label_class' => 'wcfm_title',
                    'hints'       => __('Enter the vendor\'s website URL here. This will be displayed on the PDF document sent to the customer. Leave it empty to use website URL from the vendor settings.', 'woovoucher'),
                    'value'       => $_woo_vou_website,
                ],
                '_woo_vou_how_to_use'                           => [
                    ...
                    'class'       => 'wcfm-textarea ',
                    'label_class' => 'wcfm_title ',
                    'hints'       => __('Within this option, you can enter instructions on how this voucher can be redeemed. This instruction will then be displayed on the PDF document sent to the customer after successful purchase. Limited HTML is allowed. Leave it empty to use redeem instructions from the vendor settings.', 'woovoucher'),
                    'value'       => $_woo_vou_how_to_use,
                ],
                ...
                'avail_locations'                               => [
                    ...
                    'type'        => 'multiinput',
                    'label_class' => 'wcfm_title',
                    'hints'       => __('If the vendor of the voucher has more than one location where the voucher can be redeemed, then you can add all the locations within this option. Leave it empty to use locations from the vendor settings.', 'woovoucher'),
                    'value'       => $avail_locations,
                    'options'     => [
                        '_woo_vou_locations' => [
                            ...
                            'class'       => 'wcfm-text',
                            'label_class' => 'wcfm_title',
                            'hints'       => __('Enter the address of the location where the voucher code can be redeemed. This will be displayed on the PDF document sent to the customer. Limited HTML is allowed.', 'woovoucher'),
                        ],
                        '_woo_vou_map_link'  => [
                            ...
                            'class'       => 'wcfm-text',
                            'label_class' => 'wcfm_title',
                            'hints'       => __('Enter a link to a google map for the location here. This will be displayed on the PDF document sent to the customer.', 'woovoucher'),
                        ],
                    ],
                ],
...
if ($product_id) {
    ...
    $purchased_posts_per_page = apply_filters('woo_vou_used_code_popup_per_page', 10);
    // Apply filter to change per page records
    $purchased_paged = 1;
    // Declare paged to default 1
    ...
    <div id="wcfm-woo-vou-purchased-codes-popup">
        <div class="woo-vou-popup-content woo-vou-purchased-codes-popup wcfm_popup_wrapper">
          <div style="margin-bottom: 15px;"><h2 style="float: none;"><?php _e('Purchased Voucher Codes', 'woovoucher'); ?></h2></div>
            ...
            $purchasedcodes_columns = apply_filters(
                        ...
                        if (! empty($purchasedcodes_columns)) {
                            foreach ($purchasedcodes_columns as $column_key => $column) {
                                ?>
                                    ...
                                    <th scope="row" class="<?php echo $column_key; ?>"><?php echo $column; ?></th>
                                        <?php
                            }
                        }
                        ?>
                                    ...
                                    if (! empty($purchasedcodes_columns)) {
                                        ...
                                        foreach ($purchasedcodes_columns as $column_key => $column) {
                                            ...
                                            switch ($column_key) {
                                                ...
                                                case 'buyer_info':
                                                    // buyer's info who has purchased voucher code
  ...
  <div id="wcfm-woo-vou-used-codes-popup">
        ...
        <div class="woo-vou-popup-content woo-vou-used-codes-popup wcfm_popup_wrapper">
          <div style="margin-bottom: 15px;"><h2 style="float: none;"><?php _e('Used Voucher Codes', 'woovoucher'); ?></h2></div>
            ...
            $usedcodes_columns = apply_filters(
                                ...
                                if (! empty($usedcodes_columns)) {
                                    ...
                                    foreach ($usedcodes_columns as $column_key => $column) {
                                        ...
                                        switch ($column_key) {
                                            ...
                                            case 'buyer_info':
                                                // buyer's info who has used voucher code
                                                $column_value  = '<div id="buyer_voucher_'.$voucodes_data['voucode_id'].'">';
                                                $buyer_info    = $woo_vou_model->woo_vou_get_buyer_information($orderid);
                                                $column_value .= woo_vou_display_buyer_info_html($buyer_info);
                                                $column_value .= '<a class="woo-vou-show-buyer" data-voucherid="'.$voucodes_data['voucode_id'].'">'.__('Show', 'woovoucher').'</a>';
