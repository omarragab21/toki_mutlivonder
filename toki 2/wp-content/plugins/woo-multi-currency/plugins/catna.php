	...
	public function vicatna_woocommerce_process_product_meta_simple( $post_id ) {
		...
		foreach ( $currencies as $currency => $currency_data ) {
			if ( $default_currency !== $currency ) {
				...
				update_post_meta( $post_id, $key_min, isset( $_POST[ $key_min ] ) ? wc_clean( $_POST[ $key_min ] ) : '' );
				update_post_meta( $post_id, $key_max, isset( $_POST[ $key_max ] ) ? wc_clean( $_POST[ $key_max ] ) : '' );
			}
		}
	}
	...
	public function get_fixed_price( $product_id, $is_min = true ) {
		...
		if ( $this->settings->check_fixed_price() ) {
			...
			if ( $current_currency !== $default_currency ) {
				$catna_settings = get_post_meta( $product_id, 'vicatna_settings', true );
				if ( $catna_settings['type'] === '0' ) {
					$price = get_post_meta( $product_id, '_wmc_vicatna_nyp_' . ( $is_min ? 'min' : 'max' ) . '_' . $current_currency, true );
				}
			}
		}
