...
// Disable Jetpack Photon / Site Accelerator CDN on local development
add_filter( 'jetpack_photon_development_mode', '__return_true' );
add_filter( 'jetpack_is_photon_enabled', '__return_false' );
add_filter( 'jetpack_site_accelerator_image_enabled', '__return_false' );
add_filter( 'jetpack_boost_image_cdn_enabled', '__return_false' );
...
function writeMsg($id,$post) {
$prime= get_field( 'prime', $post );
// echo $id;
// echo $prime;
if($id == 1){
...
if($prime ==='yes'){
    ...
    return'<div class="ex__type main__btn yellow_color">برايم</div>';
   
}

else{
        return'<div class="ex__type main__btn orange_color">اكسبريس </div>';
    ...
    //end admin
                ...
                return'<div class="ex__type main__btn green_color">ماركت </div>';
...
add_action('wp_enqueue_scripts', function() {
    global $WCFM;
    $version = (isset($WCFM) && is_object($WCFM) && isset($WCFM->version)) ? $WCFM->version : '1.0.0';
    wp_enqueue_style( 'wcfm-setup', get_template_directory_uri() . '/css/vendor.css', array(), $version );
});
...
if( function_exists('acf_add_options_page') ) {
    ...
    acf_add_options_sub_page(array(
        'page_title'    => 'Theme index Settings',
        'menu_title'    => 'index',
        'parent_slug'   => 'theme-general-settings',
